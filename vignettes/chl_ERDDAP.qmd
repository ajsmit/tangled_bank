...
I will provide tutorial for each of these in due module.
...
```{r fn-chl-sub-dl-function-time-df}
#| eval: false
...
```{r code-dl-years-data-frame-date-index}
#| eval: false
...
```{r code-save-chl-data-file-data}
#| eval: false
save(chl_data, file = "data/BCB744/MODIS_chl_data.Rdata")
```

Or we can download and save to disk as a netCDF (manually renamed to
`chl_data.nc` afterwards):

```{r code-griddap-datasetx-erdmh1chla1day}
#| eval: false
