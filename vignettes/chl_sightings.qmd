...
```{r code-set-opentopo-key-cd52ef9b8b407a17fd417843e571fc27}
#| echo: false
#| eval: false
# OpenTopography API
...
```{r code-sights-read-csv}
#| message: false
sights <- read_csv(
  here::here("data", "BCB744", "occurences_sampled.Mn.txt"),
...
```{r code-range-sights-date}
range(sights$date)
```

```{r code-yrs-max-year-sights}
yrs <- max(year(sights$date)) - min(year(sights$date))
```

The longitudinal and latitudinal range:

```{r code-range-sights-lat}
range(sights$lat)
range(sights$lon)
```

```{r code-median-sights-lat}
median(sights$lat)
median(sights$lon)
```
...
```{r code-dem-elevatr-get-elev-raster-locations}
#| eval: false
...
save(dem, file = here::here("data", "BCB744", "azores.dem"))
```

```{r code-load-here-here-data}
#| cache: false
# re-use previously downloaded dem
load(here::here("data", "BCB744", "azores.dem"))
...
```{r code-chldir-users-ajsmit-documents}
#| cache: false
#| eval: false
...
```{r fig-monthlycentralchlorophyll}
...
```{r fig-monthlyeasternchlorophyll}
...
```{r fn-timingsfigures}
#| fig-cap: "The annual module of sightings (D-F) and the corresponding onset of the chlorophyll-*a* maximum (A-C)."
#| out-width: "90%"
#| cache: false
#| eval: false
...
```{r code-timings-tbl-gt-timings-rownames-to-stub}
#| echo: false
#| eval: false
