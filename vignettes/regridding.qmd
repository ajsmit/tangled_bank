...
```{r code-library-tidyverse-a-staple}
...
```{r code-which-chl-ed-search-query-chlorophyll}
...
```{r code-browse-erdvh2018chla1day}
#| eval: false
...
```{r code-chl-griddap-datasetx-erdvh2018chla1day}
#| eval: false
...
```{r code-save-chl-file-volumes}
#| eval: false
#| echo: false
save(chl, file = "/Volumes/OceanData/VIIRS/VIIRS_chl.RData")
```

```{r code-load-volumes-oceandata-viirs}
#| echo: false
...
```{r code-head-unique-chl-longitude}
...
```{r code-head-unique-interp-chl-longitude}
...
```{r code-chl-st-regrid-df-as-tibble-chl-st-regrid-xy}
