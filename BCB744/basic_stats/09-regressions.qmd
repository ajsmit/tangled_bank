...
-   **Simple Linear Regression:** A statistical method used to model the linear relationship between one continuous dependent (response) variable and one continuous independent (predictor) variable, assuming causality.
-   **Regression Equation ($Y = \beta X + \alpha$):** The model is defined by its intercept ($\alpha$) and slope ($\beta$), which are estimated to minimize the sum of squared residuals (the differences between observed and predicted values).
-   **Coefficient of Determination (R²):** A key metric indicating the proportion of the variance in the dependent variable that is predictable from the independent variable. It ranges from 0 to 1.
-   **Model Assumptions & Diagnostics:** Linear regression relies on critical assumptions like linearity, independence, normality of residuals, and homoscedasticity. Diagnostic plots (e.g., Residuals vs. Fitted, Q-Q plot) are essential for verifying these assumptions.
-   **Prediction vs. Confidence Intervals:** A confidence interval estimates the range for the *average* response at a given X, while a prediction interval estimates the range for a *single future* observation, making it wider to account for individual variability.
