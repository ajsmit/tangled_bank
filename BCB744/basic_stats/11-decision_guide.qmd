...
-   **Test Selection Criteria:** The choice of a statistical test is primarily driven by the research hypothesis, the type of data (e.g., continuous, categorical), and the data's distribution.
-   **Comparing Means:** Use *t*-tests to compare the means of two groups and ANOVA to compare the means of three or more groups.
-   **Assessing Relationships:** Use correlation to measure the strength and direction of an association between two variables, or use regression to model a cause-and-effect relationship.
-   **Handling Categorical Data:** Chi-square analysis is the primary tool for testing relationships between two categorical variables.
-   **Meeting Assumptions:** Parametric tests (like *t*-tests, ANOVA, Pearson correlation) rely on assumptions like normality and homoscedasticity. If these are violated, non-parametric alternatives should be used.
...
- **[Correlations](10-correlations.qmd)**:
  - Used to examine the strength and direction of the linear relationship between two continuous variables.
    - **[Pearson's](10-correlations.qmd#pearson-correlation)**: Assumes a linear relationship and that both variables are normally distributed.
    - **[Spearman's](10-correlations.qmd#spearman-rank-correlation)**: Used when the relationship is monotonic but not necessarily linear, or when one/both of the variables are ordinal.
    - **[Kendall's](10-correlations.qmd#kendall-rank-correlation)**: Similar to Spearman's but based on the concordant and discordant pairs. Useful for smaller sample sizes or when there are many tied ranks.
...
| **[Pearson's Correlation](10-correlations.qmd#pearson-correlation)** | Measure linear association between two continuous variables.                       | Two continuous variables.                | Both variables should be normally distributed, linear relationship.                                   |
| **[Spearman's Correlation](10-correlations.qmd#spearman-rank-correlation)**| Measure monotonic relationship between two ordinal/continuous variables.           | Two ordinal/continuous variables.         | Monotonic relationship. Does not assume normality.                                                     |
| **[Kendall's Tau](10-correlations.qmd#kendall-rank-correlation)**        | Measure association between two ordinal variables.                                  | Two ordinal variables.                   | No specific distributional assumptions. Measures strength of association based on concordant/discordant pairs. |
