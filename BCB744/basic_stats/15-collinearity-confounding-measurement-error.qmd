...
The key result is therefore not only that both predictors are statistically detectable in the larger model. It is that the apparent temperature effect in the simpler model was badly exaggerated because temperature was standing in for nutrient variation. Adding nutrient improves the model very strongly (`p < 0.001` in the nested model comparison), which confirms that the one-predictor model was omitting an important process-linked variable.
...
The important point is not merely that coefficients changed. It is that the temperature-only model answered the wrong biological question because it mixed the effect of temperature with the effect of nutrient supply. In a real ecological analysis, that would make any strong mechanistic claim about temperature unreliable unless the nutrient process had also been considered.
:::
