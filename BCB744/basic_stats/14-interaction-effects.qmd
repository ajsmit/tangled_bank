...
## Short Example: Two Continuous Predictors with and without Centring

Suppose we want to model algal growth as a function of temperature and nutrient concentration, and we suspect that the effect of temperature depends on nutrient supply. With two continuous predictors, the interaction is straightforward to fit, but the uncentred coefficients can be awkward to interpret because they are defined when the *other* predictor equals zero.
...
The interaction term is the same model statement in both fits, but the main effects now answer more useful questions. In the uncentred model, the temperature coefficient is the slope when nutrient concentration is zero, and the nutrient coefficient is the slope when temperature is zero. Those values are not biologically meaningful here. In the centred model, the temperature effect is the slope at the **mean** nutrient concentration, and the nutrient effect is the slope at the **mean** temperature.

This is why centring helps in continuous × continuous interactions. It does not remove the interaction or change the fitted surface. It changes the reference point so that the main effects describe a scientifically sensible part of the predictor space.

# Example 1: Coastal distance and bioregional context
