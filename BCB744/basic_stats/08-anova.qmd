...
ANOVA tests whether variation among the group means is large relative to the variation within the groups. That comparison is expressed through the *F*-statistic:

$$F = \frac{\text{mean square among groups}}{\text{mean square within groups}}$$ {#eq-anova-f}
...
This is a scientific question in the simplest sense. Often, for basic inquiries such as that posed above, we need to see if one group differs significantly from another. The way in which we accomplish this is by looking at the mean, variance within a set of data compared against another similar set. Formally, ANOVA does this through the *F*-ratio in @eq-anova-f.
