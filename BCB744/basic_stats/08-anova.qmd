...
Analysis of variance (ANOVA) is used to *compare the means of three or more groups* to determine if there is a significant difference among them. The following expectations about your data should be met to conduct an ANOVA:
...
-   **Homoscedasticity** The variances of the groups should be approximately equal. This assumption can be tested using Levene's test, `car::leveneTest()`, or Bartlett's test, `bartlett.test()`. When the variances are different but the data are normally distributed, consider the 'Welch ANOVA,' `one.way()`, instead of `aov()`.
...
When these assumptions are met, you can perform an ANOVA to determine if there is a significant difference in the means of the groups. If your study involves multiple dependent variables, you may need to consider using multivariate analysis of variance (MANOVA) instead. ANOVAs can also accommodate multiple independent variables and are called factorial ANOVAs.
...
Once we have filtered our data we may now perform the *t*-test. Traditionally this would be performed with `t.test()`, but recent developments in R have resulted in the `compare_means()` function which some of you might come to like. We may use only this one single function for many of the tests we will perform in this chapter as well as the Chapter on [*t*-tests](07-t_tests.qmd). To use `compare_means()` for a *t*-test we must simply specify this in the `method` argument, as seen below:
...
Contrast the output with that of the standard *t*-test function, `t.test()`:
...
::: {.callout-note appearance="simple"}
## Note
If you do decide to use the `compare_means()` function, please make sure that you know exactly which test is being used! This is, *t*-test for two means, or ANOVA for more than two means. In your Methods and Results you will want to report the name of the test being used internally within `compare_means()`, i.e. `t.test()` or `aov()`. Do not say you used the `compare_means()` function---rather, know what it does!
:::
...
::: callout-important
## Task F
1. Why should we not just apply *t*-tests once per each of the pairs of comparisons we want to make?
:::
...
::: callout-important
## Task F
2. What does the outcome say about the chicken masses? Which ones are different from each other?
3. Devise a graphical display of this outcome.
:::
...
We may also produce a graphical summary (@fig-boxwhisker2):

```{r}
#| column: margin
#| label: fig-boxwhisker2
#| fig-cap: "Box-and-whisker plot showing the differences in means between chicks reared to 21 days old and fed four diets."
...
::: callout-important
## Task F
4. Look at the help file for this function to better understand what the output means.
5. How does one interpret the results? What does this tell us about the effect that that different diets has on the chicken weights at Day 21?
6. Figure out a way to plot the Tukey HSD outcomes.
7. Why does the ANOVA return a significant result, but the Tukey test shows that not all of the groups are significantly different from one another?
:::
...
::: callout-important
## Task F
8. How is time having an effect?
9. What hypotheses can we construct around time?
:::
...
::: callout-important
## Task F
10. What do you conclude from the above series of ANOVAs?
11. What problem is associated with running multiple tests in the way that we have done here?
:::
...
::: callout-important
## Task F
12. What do you conclude from the above ANOVA?
:::
...
::: callout-important
## Task F
13. What question are we asking with the above line of code? What is the answer? Also, why did we wrap `Time` in `as.factor()`?
:::
...
::: callout-important
## Task F
14. How do these results differ from the previous set?
:::
...
::: callout-important
## Task F
15. Yikes! That's a massive amount of results. What does all of this mean, and why is it so verbose?
:::
...
::: callout-important
## Task F
16. Something seems... off. What's going on here? Please explain this outcome.
:::
