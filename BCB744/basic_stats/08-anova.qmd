...
![A meme about fancy statistics.](../../images/fancy_stats.jpg){width=50%}   
...
-   **ANOVA (Analysis of Variance):** A statistical test used to compare the means of three or more groups to determine if at least one group mean is different from the others.
-   **One-Way vs. Factorial ANOVA:** One-way ANOVA analyzes one categorical independent variable (factor), while factorial ANOVA (e.g., two-way) analyzes two or more factors and their interactions.
-   **Problem of Multiple Comparisons:** Repeatedly using *t*-tests on multiple pairs of groups inflates the probability of a Type I error (a false positive). ANOVA avoids this by testing all groups simultaneously.
-   **Post-Hoc Tests (Tukey HSD):** After a significant ANOVA result (which only tells you that *at least one* group is different), a post-hoc test like Tukey's HSD is necessary to perform pairwise comparisons and identify exactly which group means differ from each other.
-   **Non-Parametric Alternative (Kruskal-Wallis Test):** If the assumptions for ANOVA (particularly normality) are not met, the Kruskal-Wallis test is used as a non-parametric equivalent.
