...
- **Parametric Assumptions:** Core requirements that data must meet for parametric tests (like *t*-tests and ANOVA) to be valid, primarily normality and homogeneity of variance (homoscedasticity).
- **Testing for Normality:** Using statistical tests, with the Shapiro-Wilk test (`shapiro.test()`) being a common choice, to formally check if a sample is likely drawn from a normally distributed population. A *p*-value > 0.05 suggests normality.
- **Testing for Homogeneity of Variance:** Using tests like Levene's test (`car::leveneTest()`) to check if the variance is equal across different groups. A *p*-value > 0.05 suggests homogeneity.
- **Handling Violated Assumptions:** When assumptions are not met, the two primary options are to either use a robust non-parametric equivalent of the test or to apply a mathematical transformation (e.g., log, square root) to the data.
- **Non-parametric Alternatives:** A class of statistical tests that do not rely on assumptions about the data's distribution, such as the Wilcoxon rank-sum test (for *t*-tests) and the Kruskal-Wallis test (for ANOVA).
...
::: {.column-margin}
![A meme about statistical assumptions.](../../images/wahlberg_assumptions.jpeg){fig-align="center" width="600"}
:::
