...
-   **Non-Parametric Tests:** A class of statistical methods that do not assume the data follow a specific distribution (like the normal distribution). They are used when the assumptions of parametric tests are violated.
-   **Rank-Based Analysis:** Instead of using raw data values, non-parametric tests often convert data to ranks and perform calculations on these ranks, making them robust to outliers and non-normality.
-   **Wilcoxon / Mann-Whitney U Test:** The non-parametric equivalent of the *t*-test, used for comparing the central tendency (medians) of two groups (paired or independent).
-   **Kruskal-Wallis Test:** The non-parametric equivalent of the one-way ANOVA, used for comparing the medians of three or more independent groups.
-   **Friedman Test:** The non-parametric equivalent for a repeated-measures ANOVA, used when comparing three or more related (non-independent) groups.
...
| ANOVA (one-way ANOVA only; ANOVAs with interactions do not have non-parametric tests) | Tests if the means of three or more independent groups are significantly different. Assumes normality, equal variances, and independence. | Continuous (DV), categorical (IV) |`aov()` | Kruskal-Wallis test |
...
| Regression Analysis | Models the relationship between two continuous variables. Assumes linearity, normality, and equal variances of errors. | Continuous (DV), continuous (IV) |`lm()` | Generalised Linear Models |
...
Here is a quick guide to apply to paired tests, one-, two-sample tests and as well as one- and two-sided hypotheses (*i.e.* *t*-tests and their ilk). Also see the [CheatSheet](../../docs/Methods_cheatsheet_v1.pdf).
