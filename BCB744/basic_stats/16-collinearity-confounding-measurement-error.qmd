...
Regression models are often taught as though the difficult part is fitting the correct formula and reading the summary table. In practice, some of the hardest challenges come afterwards. Predictors may overlap strongly, causal roles may be confused, and the variables we fit may be noisy measurements or only rough proxies for the processes we care about.

In this chapter, I bring together three closely related but distinct problems:
...
All three problems must be thought about because they impair interpretation. A model may still run and produce seemingly believable coefficients, but those coefficients may no longer mean what we would like them to mean.
...
In this chapter, I explain what collinearity is, why it is problematic, and how to diagnose and address it. I also explain why collinearity differs from confounding, and why measurement error and proxy variables create further limitations even when the fitted model looks statistically sound. While no single paper is as singularly famous here as Hurlbert's work on pseudoreplication, discussed in [Chapter 18](18-pseudoreplication.qmd), a foundational and highly recommended review of multicollinearity is that of Graham -@Graham2003.
...
The key message is that the difficulty lies in attribution, not in the mere existence of a fitted model. Once several predictors describe much the same underlying climatic gradient, the model can struggle to assign a stable partial effect to any one of them. This is why collinearity is so often an interpretive problem rather than a purely computational one.
...
We can see the logic of confounding clearly with a small biological-look-alike example. Suppose we want to know whether seawater temperature affects algal abundance along a coastal gradient, but nutrient concentration also varies with temperature and itself influences abundance. In that case, a temperature-only model risks attributing nutrient-related variation to temperature.
...
We begin by inspecting the scale of the variables and the pairwise relationships among them.
...
The EDA in @fig-confounding already suggests a confounding structure. Temperature and nutrient concentration are strongly positively correlated ($r = 0.84$), and both appear positively related to algal abundance. That means a temperature-only model would be at risk of absorbing part of the nutrient effect.

## State the Model Question and Hypotheses

The biological question is not simply whether temperature and abundance are associated. It is whether temperature still explains abundance **after nutrient concentration has been accounted for**.

$$H_{0}: \beta_{\text{temperature}} = 0$$
$$H_{a}: \beta_{\text{temperature}} \ne 0$$

Here $\beta_{\text{temperature}}$ refers to the partial effect of temperature in the model that also includes nutrient concentration. If the temperature coefficient changes strongly when nutrient is added, that is evidence that the original temperature-only result was confounded.

## Fit Competing Models

We first fit a temperature-only model, and then a second model that also includes nutrient concentration:
...
If we ignored nutrient concentration, we would conclude that temperature had a very strong positive effect on algal abundance ($\beta = 1.27$, SE = 0.09, $p < 0.001$). But once nutrient concentration is included, the estimated temperature effect shrinks sharply to about 0.25 (SE = 0.11, $p = 0.026$), while nutrient itself has a strong positive effect ($\beta = 1.33$, SE = 0.12, $p < 0.001$).

The larger model shows that the apparent temperature effect in the simpler model was badly exaggerated because temperature was standing in for nutrient variation. Both predictors remain statistically detectable, but adding nutrient improves the model very strongly (`p < 0.001` in the nested model comparison), which confirms that the one-predictor model was omitting an important process-linked variable.
...
The temperature-only model answered the wrong biological question because it mixed the effect of temperature with the effect of nutrient supply. In a real ecological analysis, that would make any strong mechanistic claim about temperature unreliable unless the nutrient process had also been considered.
:::
...
The contrast in @fig-measurement-error shows that the model using the noisy predictor typically produces a weaker slope than the model using the true predictor. This is the practical effect of attenuation bias: measurement error can make a real effect appear smaller than it is.
...
The important point is that the weaker fitted relationship is not necessarily evidence of weaker biology. It may instead reflect poorer measurement. If the predictor is noisy, the model is forced to relate the response to an imperfect version of the underlying process, and the slope is therefore pulled towards zero.
...
This workflow also forces an important distinction that connects directly to the regularisation chapter that follows later in the sequence.
...
In the next chapter, I go from model construction to the question of whether a fitted model is actually behaving well: diagnostics, comparison, and evaluation.
