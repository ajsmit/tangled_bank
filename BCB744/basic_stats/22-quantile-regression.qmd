...
-   Why the conditional mean is not always the main question
-   What quantile regression estimates
-   When upper, median, or lower trends matter
-   How quantile regression differs from ordinary regression
-   The current teaching workflow for this chapter
:::
...
When a full worked example is added, this chapter should end with the same boxed `Reporting` section used throughout the rest of the sequence. In that write-up, the `Methods` should identify the quantile(s) fitted, the `Results` should report the fitted quantile relationships, and the `Discussion` should explain what those relationships mean biologically. At present that material is deferred because the chapter is still a structured placeholder.
