...
Figures are not decoration added after the analysis is finished. In exploratory data analysis they are part of the analysis itself. A good figure lets you see shape, spread, clustering, outliers, and relationships that a table of means cannot show.

This chapter extends the summaries in [Chapter 2](02-summarise-and-describe.qmd) by turning them into graphical evidence. We will work through the most common figure types used in biological data analysis: histograms for distributions, bar graphs for counts and grouped summaries, box and violin plots for group comparisons, scatter plots for relationships, and line graphs for ordered or time-series data. Later in the site you will also encounter maps, ordination plots, and other specialised graphics, but the figures in this chapter form the core toolkit.

The examples here are deliberately basic. The aim is not to exhaust every possible `ggplot2` option, but to help you recognise which figure answers which descriptive question. For broader coverage and additional figure types, consult the following sources:
