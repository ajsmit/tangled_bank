...
This chapter extends the summaries in [Chapter 2](02-summarise-and-describe.qmd) by turning them into graphical evidence. The main task is to match the plot to the question. Some plots show the distribution of one variable, some compare groups, and some reveal relationships between variables.
...
- **Graphical EDA:** Plots are part of the analysis, not a cosmetic extra added afterwards.
- **`ggplot2`:** A flexible R package for building statistical graphics.
...
Here is the second use, where the bars represent means with standard deviations:
...
The rule to keep in mind is simple: use bar graphs for counts or proportions; use box plots or violin plots when you need to show how continuous data are distributed within groups.
...
We typically encounter line graphs in time-series displays. Points may be shown or omitted. The important feature is that the line connects consecutive observations and makes trends, cycles, and seasonality easier to see. The monthly climatology in @fig-stepped provides a simple example.
