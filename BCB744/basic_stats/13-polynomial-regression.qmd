...
In [simple linear regression](12-simple-linear-regression.qmd), we assumed that the expected response changes in a straight-line way with the predictor. In many biological settings, that first assumption is too rigid: responses often accelerate, decelerate, peak, or flatten.
...
When the biology suggests a specific process (for example a saturating uptake model), a mechanistic nonlinear model (Chapter 22) may be better. When curvature is complex and not well represented by low-order powers, a [GAM](21-generalised-additive-models.qmd) may be better.
...
-   If residual curvature remains strong, move to a more flexible model such as a [GAM](21-generalised-additive-models.qmd).
