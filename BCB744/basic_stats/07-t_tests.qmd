...
The statistical question is always broadly the same, which is, is the estimated mean, or the estimated mean difference, large enough relative to sampling variation to justify rejecting the null hypothesis? In practice, the main quantity of interest is the estimated effect itself, together with its confidence interval. The *p*-value supports that interpretation and should not replace it.

This chapter follows the same order throughout: identify the design, inspect the data, check the assumptions, run the test, and interpret the effect size, uncertainty, and *p*-value together. With only two groups, the two-sample *t*-test is the two-group special case of one-way ANOVA. [Chapter 8](08-anova.qmd) extends the same idea to more than two groups and explains why repeated *t*-tests inflate Type I error.
...
The assumptions also follow the design.
...
::: callout-important
## Do This Now!
**Task F.1.** Work with two friends and devise a real experiment, or a designed sampling exercise, in which you demonstrate the following two principles:
...
Many biological questions reduce to a mean comparison. A treatment is compared with a control. A measured trait is compared with a reference value. The same individuals are measured before and after an intervention. The scientific question changes with the design, but the inferential task remains a comparison of means or mean differences.
...
The built-in `trees` dataset records black cherry trees, with height given in feet. Here we convert height to metres and evaluate two rounded reference means, 23.0 m and 24.5 m. The example shows how the same sample can lead to different inferences depending on the reference value being tested.
...
This is a one-sample problem, so there is no grouping variable. We inspect the sample itself.
...
The assumption check is applied to the sample values themselves because this is a one-sample design. The Shapiro-Wilk test shows no strong evidence against normality for tree height, and the summary statistics do not suggest any extreme outlier dominating the sample.
...
The mean tree height is 23.16 m. Relative to 23.0 m, that difference is small and the confidence interval includes the reference value. Relative to 24.5 m, the difference is larger and the confidence interval excludes the reference value. We therefore fail to reject $H_0$ when $\mu_0 = 23.0$, but reject $H_0$ when $\mu_0 = 24.5$.

This example shows why the null value must have biological meaning. The data have not changed. The inferential conclusion changes because the reference value has changed.
...
In a two-sample design, the grouping variable has two levels and the observations in one group are independent of those in the other. The question is whether the two population means are equal.
...
If the apparent variance difference is tied to strong skewness or a long upper tail, a transformation may be preferable to a raw-scale *t*-test. [Chapter 6](06-assumptions-and-transformations.qmd) developed that diagnostic decisions, and the ozone example below demonstrates this in opractice in the final test.

# Example 2: Penguin Flipper Length in Two Species

The `penguins` dataset provides a realistic two-group comparison. Here we compare flipper length between Adelie and Chinstrap penguins. The grouping variable is `species`, and it has two independent levels in this subset.
...
First inspect the grouped summaries, the within-group normality checks, and the spread across groups.
...
The assumption checks are applied within each species because the two-sample design compares group means. The Shapiro-Wilk tests show no strong evidence against normality within either species, and Levene's test provides no strong evidence that the group spreads differ. Together with the boxplots, these checks support a standard two-group mean comparison.
...
The boxplots show a clear shift in centre, with Chinstrap penguins tending to have longer flippers. The spreads are similar. The implication is straightforward: the assumptions do not force a change of method here, so a Welch two-sample *t*-test is a defensible default.
...
The ozone example from [Chapter 6](06-assumptions-and-transformations.qmd) can now be carried through to a *t*-test for inference. We compare ozone concentrations in **May-June** with those in **August-September**. In the `airquality` dataset, ozone is recorded in parts per billion (`ppb`). On the raw scale, the data are strongly right-skewed and the group variances differ. A log transformation improves both features, so the two-sample comparison is done on the log scale and then back-transformed for reporting.
...
First inspect the grouped distributions on the raw and transformed scales.
...
Formal checks support the same conclusion:
...
This example shows the complete sequence of decision steps. It starts with diagnosing the assumption violation on the raw scale, transforming for a clear reason, re-assessing the assumptions, applying the test, and then returning the result to the original scale for reporting.
...
Before running the paired *t*-test, inspect the paired differences because these are the values used directly by the test.
...
The assumption check is applied to the paired differences, not to the raw before and after measurements. The Shapiro-Wilk test shows no strong evidence against normality in those differences, so the paired *t*-test can be used without changing method.
...
The response to assumption problems depends on the design and on the source of the problem.
...
The first test matches the direction implied by the data and the question. The second does not. Direction changes the hypothesis, the tail area, and the interpretation.
...
These procedures are often introduced as alternatives when the mean is a poor summary of the data. They should not be described mechanically as direct tests of medians in every case. Their interpretation depends on the design and on the structure of the distributions being compared.
...
# A Decision Sequence for Any *t*-Test
...
These steps prevent a large number of common mistakes.

# A *t*-Test Workflow

Now that the individual cases are in place, we can walk through a complete analysis in R. For this example we use the `ecklonia` data from [Intro R Workshop: Data Manipulation, Analysis, Graphing](https://robwschlegel.github.io/Intro_R_Workshop/).

## Loading Data

Before we can analyse anything, we need to load the data and reshape them into long format.
...
Visualise the data first so that the question is grounded in the measurements themselves.
...
The measurements occupy very different scales, which makes the full figure difficult to compare directly between sites. Its main value is orientating us to the variables in the dataset and showing that a more focused comparison is needed before a hypothesis can be stated clearly.

## Formulating a Hypothesis

Let us narrow the question to stipe mass and focus only on two sites, Batsata Rock and Boulders Beach.
...
The directional question could be stated as: are kelp stipes heavier at Batsata Rock than at Boulders Beach?
...
The response is continuous and the sites define the two independent groups. We still need to inspect spread and approximate normality within groups.
...
The output gives the test statistic, degrees of freedom, *p*-value, and confidence interval for the mean difference. If the one-sided *p*-value is small enough, we reject $H_0$ and conclude that mean stipe mass is greater at Batsata Rock. The confidence interval gives the plausible size of that difference.

## Drawing Conclusions

The scientific conclusion should ask what local ecological or hydrodynamic conditions could account for the difference in stipe mass between the two sites.
...
This example isolates two sites so that the comparison fits a two-sample *t*-test. The full dataset contains more than two sites, however, and that changes the analysis. Once the question becomes whether mean stipe mass differs across several sites, the correct next step is ANOVA rather than a sequence of pairwise *t*-tests. [Chapter 8](08-anova.qmd) develops that extension and explains why repeated two-group testing inflates Type I error.

The biological questions also expand at that point. If stipe mass differs among several sites, what combination of exposure, nutrient regime, competition, or age structure produced that pattern?

# Conclusion

*t*-tests are among the first inferential tools used in biostatistics because they connect directly to common biological questions. The main decision is design: one sample, two independent groups, or paired observations. From there, the workflow is straightforward. Inspect the data, check the assumptions, choose the appropriate test, and interpret the estimated effect with its uncertainty.
