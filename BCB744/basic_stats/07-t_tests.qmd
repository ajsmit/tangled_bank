...
- **One-Sample vs. Two-Sample vs. Paired:** The choice of *t*-test depends on the data structure: comparing one group to a known value (one-sample), comparing two independent groups (two-sample), or comparing two related measurements (paired).
- **One-Sided vs. Two-Sided Test:** A two-sided test checks for any difference ($\bar{A} \neq \bar{B}$), while a one-sided test checks for a specific direction of difference ($\bar{A} > \bar{B}$ or $\bar{A} < \bar{B}$).
- **Assumptions (Normality and Homoscedasticity):** Student's *t*-test requires normally distributed data and equal variances. Welch's *t*-test is used when variances are unequal. Non-parametric alternatives (like the Wilcoxon test) are used if normality is violated.
- **`t.test()` Function:** The primary R function for all variations of the *t*-test, controlled by arguments like `mu`, `paired`, `var.equal`, and `alternative`.
...
```{r code-compare-means-dat-sample-data-2}
compare_means(dat ~ sample, data = r_two,
