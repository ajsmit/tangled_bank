...
The mean is the arithmetic average of the data, and it is calculated by summing all the data, dividing it by the sample size and *n* (@eq-mean).

::: {.column-margin}
**The mean, $\bar{x}$, is calculated thus:**
$$\bar{x} = \frac{1}{n}\sum_{i=1}^{n}x_{i} = \frac{x_{1} + x_{2} + \cdots + x_{n}}{n}$$ {#eq-mean}
where $x_{1} + x_{2} + \cdots + x_{n}$ are the observations$n$ is the number of observations in the sample.
:::
