...
In this Chapter we will focus on the basic components of EDA, and these initial forays are built around measures of the central tendency of the data (the **mean**, **median**, **mode**) and the dispersion and variability of the data (e.g. **standard deviations**, **standard error**, **variance**, **quantiles**). I advocate a workflow in which EDA precedes inferential statistics, the topic of [Chapter 7](07-t_tests.qmd) and later.
...
In a statistical framework, a **parameter** is a fixed, unknown value that characterises a population or distribution, and is often estimated using sample data. On the other hand, a **variable** is a measurable characteristic that can vary among individuals or objects in a population or sample. Parameters are used to describe the distribution of a population, while variables are the data points that are collected and analysed to draw conclusions about the population. Variables are used to estimate parameters through statistical methods such as regression and hypothesis testing.
:::
...
-   **Scientific concept of randomness** In the scientific context, randomness refers to a specific pattern or sequence of events that completely lacks predictability, order, or regularity. The biostatistician often associates the term 'random' with *probability theory and statistical models*. In experiments, researchers use random sampling techniques to ensure that samples are representative of the population, and they employ random assignment to minimise biases in experimental groups. Randomness in statistical settings is usually quantifiable and often follows specific *probability distributions*. In computer science, algorithms generate pseudo-random numbers, which approximate true randomness but are ultimately determined by deterministic processes.
...
-   **Parametric vs non-parametric tests** If the data exhibits normality and homoscedasticity, parametric tests may be appropriate (see [Chapter 7](07-t_tests.qmd)). However, if the data have high variability or violates the assumptions of parametric tests, non-parametric alternatives may be more suitable.
