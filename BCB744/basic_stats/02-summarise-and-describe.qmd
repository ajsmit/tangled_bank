...
Exploratory data analysis (EDA) is where we learn what kind of data we actually have before we start testing hypotheses. We inspect the scale of measurement, the spread of the observations, the presence of outliers, the amount of missingness, and any obvious grouping structure. That work is not optional. A poor summary at the start usually becomes a poor analysis later.

This chapter concentrates on the basic numerical summaries used during EDA: measures of central tendency such as the **mean**, **median**, and **mode**, and measures of spread such as the **standard deviation**, **variance**, and **quantiles**. These summaries sit alongside the figures in [Chapter 3](03-visualise.qmd) and the treatment of distributions and uncertainty in [Chapter 4](04-distributions-sampling-uncertainty.qmd). Together they provide the descriptive foundation for the inferential chapters that follow.

# Key Concepts

This section isolates the main ideas before the chapter moves into definitions, examples, and worked summaries.

- **Exploratory Data Analysis (EDA):** The initial and essential process of understanding data structure, patterns, and characteristics through visual and quantitative examination.
- **Measures of Central Tendency:** Key metrics like the mean, median, and mode used to find the "center" of a dataset.
- **Measures of Dispersion:** Statistics such as standard deviation, variance, and interquartile range (IQR) that describe the spread or variability of the data.
- **Grouped Analysis:** Summarising data by different categories or groups to reveal underlying patterns and relationships between variables.
- **Data Distribution:** The shape of the data (e.g., normal, skewed) which dictates the appropriate choice of summary statistics and subsequent statistical tests.

# Foundational Definitions

Before moving into numerical summaries, we need a few definitions that recur throughout the module.
...
The `glimpse()` function is not dissimilar to `print()` as it gives more-or-less the same kind of output, but with a horizontal orientation:
...
Tibbles do not have row names, but dataframes can have (although they are not frequently used). If we *did* have row names and wanted to know their names, we could use `row.names()` (note the `.` in the function name). We can also use `colnames()` (`names()` returns an identical output)`row.names()` to assign new names to the dataframes or tibbles --- see the respective help files for more information about this use. There is also the `dimnames()` function.
...
The mean is the arithmetic average of the data. As shown in @eq-mean, it is calculated by summing all the observations and dividing by the sample size, $n$.
...
The median is therefore the value that separates the lower half of the sample data from the upper half. In normally distributed continuous data the median is equal to the mean. Comparable concepts to the median are the **1st** and **3rd quartiles**, which, respectively, separate the first quarter of the data from the last quarter --- see later in the section on measures of variance and dispersal in this chapter. The advantage of the median over the mean is that it is unaffected by extreme values or outliers. The median is also used to provide a robust description of non-parametric data (see [Chapter 4](04-distributions-sampling-uncertainty.qmd) for a discussion on normal data and other distributions).
...
I have seldom used the concepts of skewness or kurtosis in routine EDA, but it is worth being aware of them. One purpose of examining data using measures of central tendency is to get an idea of whether our data are **normally distributed**. See [Chapter 4](04-distributions-sampling-uncertainty.qmd) for a discussion of data distributions. These descriptive statistics serve well as a first glance, but they do not replace assumption checks. We will return to formal testing and model assumptions later in the module.
...
Understanding variability matters because the same mean can arise from very different datasets. A sample with tight spread around the mean tells a different biological story from one with the same mean and far greater scatter. Variation also affects precision, group comparison, and the plausibility of later inferential assumptions such as equal variances. High variability can hide real structure; low variability can make patterns much easier to detect. It also affects design. If the data are noisy, larger sample sizes are usually required to detect a biologically meaningful effect.

Variation therefore helps determine which summaries and models are sensible. The mean and standard deviation are useful when the data are roughly symmetric and not dominated by outliers. The median and interquartile range are often better when the data are skewed or contain extremes. In later chapters the same logic will matter again when we decide between parametric and non-parametric procedures, or between simple models and models that explicitly accommodate unequal variance or complex structure.

Let us now look at the main measures of spread.

### Variance and standard deviation

Variance and standard deviation (SD) are examples of interval estimates. The sample variance is given in @eq-var, and the standard deviation in @eq-sd. If we cannot be bothered to calculate them by hand, we may use the built-in functions `var()` and `sd()`:
...
The interquartile range (IQR) is a measure of statistical dispersion that provides information about the spread of the middle 50% of a dataset. As shown in @eq-iqr, it is calculated by subtracting the first quartile (25th percentile) from the third quartile (75th percentile).

The quartiles and IQR are useful because they focus on the middle half of the data rather than on the extremes. The median (the second quartile) gives a centre that is much less sensitive to outliers than the mean. The IQR then measures the spread of the central 50% of the observations. That makes it especially useful for skewed data, for datasets with a few extreme values, and for comparisons where the standard deviation would be overly influenced by outliers. The same quantities also underpin box plots, where they provide a compact visual summary of centre, spread, and potential outliers.
...
And lastly, there is the **summarytools** package, the`dfSummary()` function within:
...
Whole-dataset summaries are only a starting point. Biological data are often structured by species, treatment, site, sex, season, or year, and those groupings can matter more than the overall mean. Once we acknowledge that structure, the descriptive question changes from "What is the average?" to "Average for whom, under what condition, and with what spread?"
...
This is still a descriptive result rather than a formal inferential test, but it shows why grouped summaries matter. Once the data are split by diet and time, biologically important differences become visible that would be hidden in a single overall mean.
:::
