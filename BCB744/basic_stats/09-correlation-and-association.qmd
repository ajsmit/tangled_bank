...
In the previous chapters, I asked whether means differ among groups. Correlation answers a different question: do two variables vary together?

Here we concern ourselves with association rather than group comparison. It is the last of the staple inferential tools and prepares the transition into regression. Correlation does not impose a response model. It measures the strength and direction of association between two variables. Regression goes further by assigning one variable the role of response and estimating how it changes with a predictor.

Correlation coefficients are effect sizes. Their sign shows direction and their magnitude shows how tightly the variables vary together. Coefficients vary from `-1.0` (perfect inverse correlation), to `0` (no association), to `1.0` (perfect positive correlation). A coefficient of `0.4` may be unremarkable in one system and biologically substantial in another, so it is highly context specific. Sample size also affects stability because correlation estimates are less stable in small datasets.
...
The scatterplot comes first. Decide whether the pattern is:
...
Those labels are only heuristics. A moderate correlation may be biologically useful if the variables are noisy or hard to measure. A strong correlation may still be uninformative if it is driven by site structure, repeated measurements, or a lurking third variable.
...
We begin by asking whether the length of *Ecklonia maxima* stipes tends to increase together with frond length.
...
The scatterplot shows a clear positive linear trend. The points are not tightly packed around the fitted line, but the relationship is straight enough that Pearson's coefficient is appropriate. The main diagnostic here is linear form. Univariate normality is secondary.
...
Suppose the question is whether longer primary blades also tend to be wider in the same rank order. This is a direct concordance question, which is the natural setting for Kendall's $\tau$.
...
These matrices are exploratory. They help identify strong positive and negative relationships, but they also create many pairwise comparisons at once. With many variables, some correlations will appear by chance. A matrix should therefore guide further inspection and not be used as a finished inferential result.
...
This is why independence is a primary condition, not a secondary detail. A naive pooled coefficient mixes at least two sources of structure:
...
If the question is "do these variables co-vary?", correlation is appropriate. If the question is "how much does the response change when the predictor changes?", the analysis has already moved into regression.
