...
The practical aim is not to prove that a model is perfect. It is to decide whether the fitted model is adequate for the scientific question being asked — good enough that the inferences it produces can be defended.
...
The structure of what follows mirrors the conceptual split above. Sections 5.1 through 5.3 address model checking — residual structure, assumption limits, and influence. Sections 5.4 through 5.6 address model evaluation — model comparison and generalisation.

## 5.1 Residual Diagnostics

Residual plots are the natural first step in model checking. They reveal whether systematic structure remains after fitting and whether the distributional assumptions are broadly satisfied.
...
These four standard plots allow assessment of several things simultaneously:
...
The practical points are these. Tiny deviations from normality are usually unimportant — regression inference is robust to mild non-normality, especially in larger samples. Large datasets make this worse in the opposite direction: even trivial deviations from the ideal distribution will produce a highly significant formal test result, even when the deviation has no practical consequence for inference. Visual residual structure is usually more informative than a single assumption test precisely because a plot conveys the pattern and its magnitude simultaneously.

The real question is whether the model is adequate for the inferential goal, not whether it is mathematically perfect.

There is one further limitation worth stating explicitly. Residual diagnostics cannot reveal problems that were introduced at the predictor level. A model with highly collinear predictors will typically produce residuals that look perfectly well-behaved — the collinearity is hidden in the instability of the individual coefficients, not in the residual structure. Similarly, measurement error in a predictor attenuates the estimated slope but leaves the residuals looking reasonable. Model checking must therefore be interpreted alongside the assessments in [Chapter 16](16-collinearity-confounding-measurement-error.qmd). Clean diagnostics indicate that the model is adequate for the data it was given; they do not guarantee that the data were right for the question.

## 5.3 Influence and Leverage

Not all unusual observations matter equally.

A point with a large **residual** is poorly fitted — the model does not predict it well. A point with high **leverage** occupies an extreme position in predictor space — it has an unusual combination of predictor values. A point with high **influence** materially changes the fitted coefficients when it is included or excluded.
...
An influential point should not be deleted automatically. It may reflect a data entry error, a measurement problem, a biologically informative extreme case, or a model that is too simple for the pattern in the data. The right response depends on which of these it is, which requires investigation rather than mechanical trimming.

## 5.4 Model Comparison

Biological analysis often involves several plausible models. Model comparison should therefore be framed as comparing **competing explanations**, not as a numerical optimisation problem.

**AIC (Akaike Information Criterion)** compares models based on their in-sample fit penalised for complexity: models with more parameters are penalised because they explain the present data at the cost of fitting more noise. A lower AIC is preferred, but the scale is relative — only differences between models matter, not the raw value.

We compare three models from the seaweed example:
...
The selected model is far better than the null model — the predictors explain substantial variation in Sørensen dissimilarity. Its AIC is competitive with the larger model while remaining simpler. That is an important general lesson: a more complex model is not automatically a better scientific model, especially when the complexity carries collinear predictors whose coefficients cannot be cleanly interpreted.

## 5.5 Overfitting and Generalisation

A model can fit the current data very well while performing poorly on new data. This is **overfitting** — the model has learned specific features of this dataset, including its noise, that do not generalise.

Overfitting is especially likely when the sample size is modest relative to the number of predictors, when interaction terms are added freely, or when model choice is heavily data-driven rather than theory-guided. This is one reason why the stepwise selection approaches that dominated earlier statistical practice are now viewed with scepticism: they optimise in-sample fit through a process that naturally finds the noise.

The key distinction is between **in-sample fit** (how well the model describes the data used to estimate it) and **generalisation** (how well it would perform on genuinely new data). These can diverge substantially.
...
The approach is straightforward: divide the data into parts, fit the model on all-but-one part, test it on the held-out part, and repeat. The prediction error averaged across all held-out portions is a direct estimate of how well the model performs on new data.

**AIC and cross-validation answer different questions.** AIC compares models on in-sample fit penalised for complexity: it is a model selection tool for the current dataset. Cross-validation estimates how well a fitted model would perform on data it has not seen: it is a generalisation tool. Both are useful, but they are not interchangeable. A model with the lowest AIC is not necessarily the best predictor on new data, and a model that cross-validates best may not have the most interpretable coefficients.
...
## 5.7 What the Diagnostics Tell Us
...
6.  balance fit, complexity, and interpretability — the best-fitting model is not always the most useful one;
