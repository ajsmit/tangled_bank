...
-   When a curved linear-model fit is appropriate
-   How quadratic and cubic terms are specified
-   How to interpret fitted curvature
-   The limits of polynomial models
:::
...
Equation @eq-poly shows a cubic model. The important point is that the model is still linear in the coefficients even though it is curved in the predictor.
...
Once a full worked example is developed for this chapter, it should end with the standard `Reporting` section used throughout the sequence. That section should include a boxed *Marine Biology* style write-up with separate `Methods`, `Results`, and `Discussion` components. For now, the omission is intentional because the chapter does not yet contain a complete analysed example.
