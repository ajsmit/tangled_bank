...
-   **Model selection** is the process of choosing among competing specified models. It follows specification; it does not replace it.
...
-   **ANCOVA** combines a factor and a continuous covariate in one additive linear model. It is not a separate procedure; it is a special case of multiple regression.
...
This is a specification exercise — there is no single right answer, but every choice must be justifiable.
:::
...
-   **No severe multicollinearity:** Predictors should not be so strongly intercorrelated that their partial coefficients become imprecise or uninterpretable. The variance inflation factor quantifies this.

Even when residual assumptions are satisfied, the model can still be scientifically weak if the variables are chosen without biological justification. Statistical adequacy and scientific defensibility are separate criteria and both are indespensible to model building.

::: callout-important
## Do It Now!

Load the seaweed dataset and produce a pairs plot of the five candidate predictors and the response `Y` for the ECTZ subset. Look for approximate linearity and for strong correlations among predictors.
...
This expands to `predictor1 + predictor2 + predictor1:predictor2`. The main effects are always included when an interaction is specified this way.
...
Reproduce the five individual simple regressions shown in the figure above, but extract the $R^2$ and slope from each using `broom::glance()` and `broom::tidy()`. Then rank the predictors from strongest to weakest marginal association with `Y`.
...
map_dfr(predictors, function(p) {
  mod <- lm(as.formula(paste("Y ~", p)), data = sw_ectz)
  tibble(
    predictor  = p,
    slope      = tidy(mod)$estimate[2],
    r_squared  = glance(mod)$r.squared,
    p_value    = glance(mod)$p.value
  )
...
The null hypothesis for the overall model is that none of the entered climatological variables explains variation in `Y`:
...
These individual tests must be read as partial tests: each evaluates the contribution of that predictor given the others in the model.

## Fit the Initial Model

I fit a full model containing all five climate predictors and a null model containing only the intercept.

The **null model** (`lm(Y ~ 1, data = sw_sub1)`) estimates a single parameter: the grand mean of `Y`. It contains no predictors, so its fitted value is identical for every observation and its residuals are the deviations of each observation from that mean. The null model is the baseline against which any more fully-specified model is compared: a nested test asks whether adding predictors reduces residual variance by more than chance alone.
...
::: {.callout-note appearance="simple"}
`null_mod1` is not passed explicitly to `summary()` or any other function in the code block above. `summary()` on a fitted `lm` object computes the overall $F$-statistic internally, by comparing the fitted model's residual sum of squares with the total sum of squares around the grand mean of `Y`. That total sum of squares is what the null model's residuals would give, and R derives it directly from the response vector without needing the null model object. The explicit `null_mod1` is therefore fitted here for two reasons: to make the conceptual baseline visible as a named model object, and to allow a direct nested comparison using `anova()`, which is demonstrated below.
:::

Calling `anova(null_mod1, full_mod1)` makes the nested comparison clear by asking whether adding all five climate predictors to the intercept-only baseline reduces residual variance by more than would be expected by chance.
...
The following iterative procedure removes the predictor with the highest VIF at each step until all remaining VIF values fall below a threshold of 10.
...
Not all three retained predictors may be needed to explain the response. I use forward stepwise AIC to identify the most parsimonious model within the reduced candidate set.

VIF screening and stepwise AIC address different problems and must be applied in that order. VIF screening is a stability check; it asks whether the coefficient estimates are numerically reliable given the intercorrelations among the predictors. If two predictors share most of their information, their partial coefficients become imprecise and their standard errors inflate, regardless of how well either predicts the response.

AIC-based selection cannot fix this: it will choose among poorly identified models and may retain collinear predictors simply because their combined redundancy happens to reduce the AIC criterion slightly. Removing collinear predictors first produces a candidate variable set in which every remaining predictor carries reasonably independent information. Selection then operates on a set of well-identified models and the AIC differences between them are interpretable. Applying `stepAIC` to the original five-predictor set, before VIF screening, would risk retaining a subset of correlated predictors whose coefficients look stable in the selected model only because the worst offenders were excluded by chance during the search.
...
## Reading the Coefficient Table and the ANOVA Table
...
The **ANOVA table** from `anova()` is *sequential*. It partitions the total sum of squares in the order the predictors were entered: `augMean` is assessed first, then `febSD` gets credit for what `augMean` left unexplained, and `augSD` gets the remainder. Change the order of predictors in the formula and the sums of squares in the ANOVA table change. For testing whether a predictor contributes after adjusting for all others, the coefficient table is the appropriate place to look. The ANOVA table is more informative when the question is about the total variance attributable to each predictor in a natural sequence, or when comparing nested models (see [Chapter 15](15-interaction-effects.qmd)).
...
## A Note on Scaling and Centring
...
The climate-plus-bioregion additive model fitted the data substantially better than the corresponding climate-only model (lower AIC; nested model comparison $p < 0.001$). August mean temperature remained the dominant positive predictor. The inclusion of bioregion showed that regional differences in baseline dissimilarity persisted even after the climate variables were accounted for. The overall model explained a large proportion of the variation in mean Sørensen dissimilarity.
