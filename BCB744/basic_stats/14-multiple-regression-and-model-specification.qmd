...
::: {.callout-note appearance="simple"}
`summary()` on a fitted `lm` object computes the overall $F$-statistic internally, by comparing the fitted model's residual sum of squares with the total sum of squares around the grand mean of `Y`. That total sum of squares is what the null model's residuals would give, and R derives it directly from the response vector without needing the null model object. I explicitly fit the `null_mod1` here because I want to: i) make the idea of the null model visible as a named model object, ii) allow a direct nested comparison using `anova()`, which I will demonstrate below, and lastly, iii) to apply in the `stepAIC()` forward model selection procedure, which I also demonstrate below.
:::
...
sw_sub2 <- sw |>
  ...
  dplyr::mutate(bio = relevel(factor(bio), ref = "AMP"))
