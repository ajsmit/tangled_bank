...
In @eq-gam, each $f_j$ is a smooth function estimated from the data rather than a straight-line coefficient multiplying the predictor directly. This is the key structural change relative to an ordinary linear model.
...
# Example 1: Sea-Temperature Structure Through Time
...
```{r code-data}
#| tbl-cap: "A subset of the Port Nolloth temperature time series used in the GAM example."
temp_raw <- read_csv(here::here("data", "BCB744", "SACTN_day_1.csv"), show_col_types = FALSE)
...
The data in @fig-temp-series and @fig-temp-season show clear nonlinearity through time and a strong seasonal cycle. A straight-line model in time is therefore likely to be inadequate.
...
`gam.check()` and the plots in @fig-gam-check help assess residual behaviour and whether the chosen basis dimensions (`k`) are adequate.
...
The smooths in @fig-gam-smooths show that `t_index` captures gradual long-term structure that a single linear slope cannot represent. The cyclic month smooth captures recurring within-year seasonality without forcing identical month effects in each year. The fitted values in @fig-gam-fit show how those smooth components reconstruct the observed pattern through time.
