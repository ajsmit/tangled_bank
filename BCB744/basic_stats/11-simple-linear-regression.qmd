...
In its simplest form, the fitted model is written as:

$$Y_i = \alpha + \beta X_i + \epsilon_i$$ {#eq-slr}

where $Y_i$ is the response for observation $i$, $X_i$ is the predictor, $\alpha$ is the intercept, $\beta$ is the slope, and $\epsilon_i$ is the residual error.
...
The journal-style Results statement for this worked example might read as follows:
