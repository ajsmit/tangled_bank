...
If the biological question is whether the covariate-response slope differs among groups, then the model is no longer a simple ANCOVA in the introductory sense. It becomes an **interaction model**, and that is the topic of [Chapter 14](14-interaction-effects.qmd).
...
The first two questions are additive-model questions and therefore belong in this chapter. The third question asks whether the slope of distance differs among bioregions, which is an interaction question and is therefore deferred to [Chapter 14](14-interaction-effects.qmd).
...
At this stage we are still fitting the simpler model in which distance has one common slope. That keeps the interpretation aligned with the main goal of Chapter 13: understanding how continuous and categorical predictors coexist in one additive linear model. In [Chapter 14](14-interaction-effects.qmd), we return to the same biological setting and ask whether allowing the slope of distance to differ among bioregions improves the model further.
