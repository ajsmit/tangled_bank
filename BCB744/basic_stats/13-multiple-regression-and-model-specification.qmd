...
In @eq-mlr, each $\beta_j$ is a partial slope. That means each coefficient is interpreted while holding the other predictors in the model constant.
...
# Example 3: A Richer Final Model
...
> A richer interaction model was fitted to evaluate whether the relationships between Sørensen dissimilarity and the selected climate predictors differed among bioregions. The interaction-rich model was compared with the corresponding additive model using information criteria and nested model comparison.
...
> This richer model supports the broader ecological interpretation that the climatic drivers of compositional turnover are not spatially uniform along the South African coast. In a manuscript, that would be the main discussion point rather than a long catalogue of coefficient values.
:::
