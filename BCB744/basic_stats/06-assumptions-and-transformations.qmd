...
Parametric tests such as *t*-tests, ANOVA, correlation, and linear regression work well when the data and design satisfy certain conditions. Those conditions are not technical footnotes. They determine whether the inferential output is trustworthy.

This chapter deals with two practical questions. First, how do we check whether the assumptions are adequate for the method we want to use? Second, what do we do when they are not? Sometimes the answer is to use a non-parametric alternative. Sometimes it is to transform the response, sometimes to change the model, and sometimes to admit that the design itself does not support the intended inference.
...
Across the methods covered in the surrounding chapters, we usually need to ask whether the data satisfy a small set of recurring conditions:
...
The last two points are closer to design requirements than to assumptions in the narrow sense. No transformation can rescue non-independent observations or turn the wrong kind of response variable into the one required by the method. Those problems have to be addressed at the level of design or model choice.
...
The null hypothesis for a normality test is that the data are not detectably different from a normal distribution:

$$H_{0}: \text{the distribution of the data is normal}$$
...
The hypotheses for equality of variances can be written in either a two-sided or a one-sided form. The two-sided version is usually the most relevant in introductory work:
...
Here $\sigma^{2}_{A}$ and $\sigma^{2}_{B}$ are the variances for samples $A$ and $B$, respectively.
...
The non-parametric substitutes for the parametric tests we often use are discussed within the relevant method chapters: Wilcoxon procedures in [Chapter 7](07-t_tests.qmd), Kruskal-Wallis in [Chapter 8](08-anova.qmd), and rank-based correlation in [Chapter 9](09-correlation-and-association.qmd). [Chapter 10](10-test-selection.qmd) then summarises how those choices fit together. Transformations are the other major response, and we consider them in the rest of this chapter.
