...
In this chapter, I organise the discussion around data structure. Group comparisons require checks within groups and across group variances. Paired designs require checks on the differences. Associations require checks on the joint pattern of the variables.
...
In the regression chapters, I apply these same ideas to **residuals**, which are the differences between observed values and model predictions. In this chapter, I develop the assumptions at the level of raw data. The extension to residuals belongs with the regression series of chapters.
:::
...
In this chapter, I stop at the diagnostic stage. The inferential step belongs in [Chapter 7](07-t_tests.qmd), where the same ozone data are analysed with a two-sample *t*-test on the transformed scale and the result is then back-transformed for reporting.
