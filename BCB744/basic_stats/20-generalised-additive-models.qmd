...
-   Why GAMs are useful for complex smooth relationships
-   How GAMs differ from polynomial and nonlinear models
-   The role of smooth functions in model fitting
-   When flexibility is useful and when it is excessive
-   The current workflow for developing and interpreting a GAM
:::
...
In @eq-gam, each $f_j$ is a smooth function estimated from the data.
...
When the worked example is added, this chapter should also conclude with the standard boxed `Reporting` section used elsewhere in the Biostatistics sequence. That write-up should separate the model specification, fitted smooths, and biological interpretation into `Methods`, `Results`, and `Discussion` respectively. The absence of that section here reflects the placeholder status of the chapter rather than a different reporting standard.
