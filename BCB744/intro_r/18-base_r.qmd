...
```{r code-x-assign-to-x, echo=TRUE, error=TRUE}
...
```{r code-is-integer-k-is, echo=TRUE, error=TRUE}
...
```{r code-y-as-integer, echo=TRUE, error=TRUE}
y <- as.integer(13)
y
class(y)
...
```{r code-z-as-integer-pi, echo=TRUE, error=TRUE}
z <- as.integer(pi)
z
class(z)
...
```{r code-x-y-sample-values, echo=TRUE, error=TRUE}
...
```{r code-u-true-v-false, echo=TRUE, error=TRUE}
...
```{r code-help, echo=TRUE, error=TRUE}
help("&")
```
...
```{r code-as-integer-true-the, echo=TRUE, error=TRUE}
...
```{r code-d-as-character-pi, echo=TRUE, error=TRUE}
d <- as.character(pi)
class(d)
```
...
```{r code-a-fluffy-b-bunny, echo=TRUE, error=TRUE}
a <- "fluffy"; b <- "bunny"
paste(a, b)
paste(a, b, sep = "-")
```
...
```{r code-lonely-vector-intro-data-na-perc}
...
```{r code-lonely-vector}
lonely_vector
```

Let us create some vectors of our own:

```{r code-primes1-c, echo=TRUE, error=TRUE}
primes1 <- c(3, 5, 7)
primes1
class(primes1)
...
```{r code-tf-c-true-false, echo=TRUE, error=TRUE}
...
```{r code-primes12-c-primes1-primes2, echo=TRUE, error=TRUE}
...
```{r code-nonsense-find-the-seventh, echo=TRUE, error=TRUE}
...
```{r code-a-c, echo=TRUE, error=TRUE}
a <- c(2, 6, 3, 8, 13)
a
a[-3]
a[10]
...
```{r code-seq-assign-them-to, echo=TRUE, error=TRUE}
...
```{r code-a-c-2, echo=TRUE, error=TRUE}
a <- c(1, 3, 5, 7)
b <- c(1, 2, 4, 8)
```
...
```{r code-a-b, echo=TRUE, error=TRUE}
a * b
```
...
```{r code-v-rep-len, echo=TRUE, error=TRUE, warning=TRUE}
v <- rep(2, len = 13)
u <- rep(c(1, 20), len = 5)
v + u
```
...
For this task, please create two 'stackable' dataframes, but each with a different number of rows. Then stack them, assign an 'index column' so that the two original dataframes can easily be distinguished from one-another in the combined dataframe.
\end{mybox}
...
```{r code-round-mean-intro-data-depth}
