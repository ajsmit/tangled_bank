...
CSV stands for 'Comma Separated Value'. A CSV file is a simple text file that stores data in a tabular format, with each row representing a record, each column representing a field of data. In a CSV file, each data value is separated by a comma (or sometimes another delimiter such as a semicolon or tab), and each row is terminated by a new line. 
...
CSV files are easy to create and use, and they have the advantage of being lightweight, easy to read and write by both humans and machines. However, they can be limited in their ability to represent complex data structures or to handle large amounts of data efficiently. Additionally, if our data contain certain kinds of special characters, this can cause problems with parsing the file correctly.
...
In a TSV file, the fields are separated by tab characters (represented as `\t` in many programming languages). This format is particularly useful when dealing with data that include commas within the values, as it avoids potential conflicts, parsing issues.

CSV files are more common and widely supported than TSV files. However, they can present difficulties when the data itself contains commas, potentially causing confusion between actual field separators, commas within the data. To mitigate this issue, values containing commas are often enclosed in quotation marks.

Like CSV files, TSV can also be imported into, exported from spreadsheet software like Excel, and read and manipulated using programming languages like Python and R. The choice between TSV and CSV largely depends on the nature of the data and personal preferences, but it is crucial to be aware of the delimiter used in order to accurately parse the files. The same functions that read or write CSV files in R can be used for TSV, but one has to set the arguments `sep = "\t"` or `delim = "\t"` for the functions `read.csv()` and `read_csv()` respectively.
...
-   **Gene name errors** In 2016, researchers discovered that Excel automatically converted gene symbols to dates, floating-point numbers. For example, gene symbols like SEPT2 (Septin 2) were converted to "2-Sep" and gene symbols like MARCH1 (Membrane Associated Ring-CH-Type Finger 1) were converted to "1-Mar". This led to errors and inconsistencies in genetic data, affecting nearly 20% of published papers in leading genomic journals.

-   **Reinhart-Rogoff controversy** In 2010, economists Carmen Reinhart, Kenneth Rogoff published a paper arguing that high levels of public debt were associated with lower economic growth. Their findings influenced policy decisions worldwide. However, in 2013, other researchers found that Reinhart, Rogoff's results were affected by an Excel spreadsheet error that excluded some data points, causing them to overstate the relationship between debt and growth.
...
-   **Issues with floating-point arithmetic** Excel uses floating-point arithmetic, which can cause rounding errors, imprecise results when working with very large or very small numbers. These inaccuracies can lead to incorrect conclusions or predictions in data analysis and scientific research.
:::
...
**Be a good, decent human, turn on file extensions on our computers.**

To display file extensions on your computer, you will need to adjust the settings in your operating system's file explorer. The specific steps to do this vary depending on whether you are using Windows, macOS, or a Linux-based system. A quick online search will provide you with the necessary instructions. Since most people use Windows, I have included the instructions in the box "Displaying file extensions in Windows".
...
When writing code in support of a statistical data analysis in R, it is important that you adhere to code conventions to ensure your code is tidy, reproducible, and unambiguous. Here are some general guidelines to follow:

-   **Use a consistent coding style** Adopt a consistent coding style throughout your codebase to make it easy for others to read and understand your code. R has a widely-accepted style guide called [The tidyverse style guide](https://style.tidyverse.org/), which you can follow. Also peruse [R Style Guide](http://adv-r.had.co.nz/Style.html) and [Google's R Style Guide](https://google.github.io/styleguide/Rguide.html) with opinionated views of how to use spaces around `<-`, after `#`, after `, `, and various other stylistics principles, with more guidelines you can adopt if you want to produce well-structured, well-formatted, and legible code.

-   **Write meaningful variable and function names** Choose clear, descriptive names for your variables, functions that reflect their purpose. I use lower-case letters and separate words with underscores (e.g.`average_temperature`), but other options are available---see "Naming Variables" below.
...
-   **Comment and document your code** Provide comments throughout your code to explain the purpose of particular sections, especially when the code is complex, not self-explanatory. Use the`#` symbol to write single-line comments.

-   **Modularise your code** Break your code into smaller, reusable functions that perform specific tasks. This makes your code more manageable, easier to understand.
...
-   **Be mindful of absolute vs. relative paths** Use relative paths whenever possible, as this makes your code more portable, reproducible across different systems.
...
It is important to note that different programming languages and communities may have their own conventions, and it is always a good idea to follow the established conventions to make your code more readable, understandable for others. But and whatever you decide to use, be consistent throughout your code. By following guidelines consistently, you will produce clean, well-organised code that is easy to understand, maintain and which is crucial for any data analysis project. 
...
In programming, variable names are an essential aspect of writing readable, maintainable code. Good variable naming conventions can make code easier to understand and can even help prevent bugs by making it clear what the variable represents. Here are some commonly used naming conventions and their examples:

-   **Pascal Case** (also known as upper camel case) is a convention where the first letter of each word in a variable name is capitalised. This is often used in object-oriented programming to name classes and types. Examples of Pascal case are `SpeciesName`, `PetalColour`, `FunctionalType`.

-   **Snake Case** (also known as 'snake_case' or 'lowercase with underscores') is a convention where words in a variable name are separated by underscores and all letters are lowercase. This is commonly used in scripting languages and file naming conventions. Examples of Snake case are `species_name`, `petal_colour`, `functional_type`.

-   **Camel Case** (also known as 'lower camel case') is a convention where the first letter of each word in a variable name is capitalised, except for the first word. This is commonly used in object-oriented programming to name variables, methods. Examples of Camel case are`speciesName`, `petalColour`, `functionalType`.

-   **Hungarian Notation** is a naming convention where a prefix is added to the variable name to indicate its data type. This convention is less commonly used in modern programming languages. Examples of Hungarian Notation are `iSpeciesID` (where 'i' indicates an integer), `strPetalColour` (where 'str' indicates a string), `fSeawaterTemperature` (where 'f' indicates a floating-point number).
:::

# Reproducible research

Reproducible research is a critical aspect of scientific integrity and advancement and it is increasingly being applied in the fields of biological and ecological sciences. These disciplines often involve complex analyses and data sets that require meticulous documentation with meta-data, and validation. Reproducible research ensures that results can be independently verified, repeated at a later date by our future selves, the reader audience of scientific publications and thereby improving transparency and minimising errors and biases. An example of some serious consequences stemming from lack of clear communication and transparency is presented in the box "The Mars Climate Orbiter mission in 1998". Reproducible research practices foster collaboration, and promote the growth of scientific knowledge.
...
-   **Version control** Integrating R projects with version control systems like [Git](https://git-scm.com/) and platforms like [GitHub](https://github.com/) or [GitLab](https://about.gitlab.com/) allows us to track changes in our code, share our work, and collaborate more effectively with our colleagues. This practice enables a clear record of code evolution, ensures the reproducibility of analyses across different stages of a project.
...
-   **Dynamic report generation** R Markdown and Quarto documents can be converted into various output formats, such as HTML, PDF, or Word, facilitating the creation of dynamic, interactive reports, presentations. This feature ensures that any changes to our data or analyses are automatically updated in the final output and maintaining the consistency and accuracy of the results.

-   **Workflow management** Tools like [**workflowr**](https://workflowr.github.io/workflowr/index.html) and [**targets**](https://docs.ropensci.org/targets/) provide us with robust and efficient workflow management solutions for large and complex projects. These packages streamline the process of organising, executing, and caching analytical pipelines, making it easier to reproduce, validate our results.

-   **Containerisation** Although beyond the scope of BCB744, it is worth being aware of technologies such as [Docker](https://www.docker.com/) that we can use in conjunction with R, RStudio to create isolated and reproducible environments with specific package versions and dependencies. This enhances portability and ensures that our analyses can be consistently reproduced across different systems.
...
-   **Code testing and validation** Packages like [**testthat**](https://testthat.r-lib.org/) enable us to develop and execute 'unit tests' for R functions, ensuring the reliability, accuracy of our code. This helps us to identify and resolve potential issues before they impact the overall reproducibility of our research findings.
