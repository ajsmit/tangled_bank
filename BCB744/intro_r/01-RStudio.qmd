...
- Understanding the basic structure of an R script
- Installing R and R Studio
- Overview of the R Studio interface
- Basic operations in R (e.g. arithmetic, variables, data types, etc.)
- Introduction to the R Console and R script editor
...
- Integrated console for running R code
- Graphical interface for managing packages and libraries
- Integrated tools for plotting and visualization
- Support for version control with Git and SVN

In summary, R is the core software for statistical computing, like a car's engine, while RStudio provides a more user-friendly interface for working with R, like the car's bosy, the seats, steeting wheel, and all the other bells and whistles.
...
::: callout-important
## Task A
1. Why is it best practice to include packages you use in your R program explicitly?
:::
...
::: callout-important
## Task A
2. What are the values after each hashed statement in the following? Use the RStudio **Console** to determine these values:
...
3. Use R to calculate some simple mathematical expressions entered. Assign the value of `40` to `x` and assign the value of `23` to `y`. Make `z` the value of `x - y` Display `z` in the console.
:::
...
```{r ggplot2-1, fig.cap = "A plot assembled with __ggplot2__."}
library(tidyverse)
x <- seq(0, 2, by = 0.01)
y <- 2 * sin(2 * pi * (x - 1/4))
ggplot() +
