...
You have already seen the basic tidy workflow: import data, inspect it, tidy it, analyse it, and plot it. This chapter focuses on the transformation stage. Here you learn the core `dplyr` verbs well enough that they become routine rather than mysterious.
...
2. The output is a data frame.
3. The result stays tidy unless you *deliberately* collapse it (mainly with `summarise()`).
...
The pipe organises your reasoning. Each line should answer one question:
"What rows do I want?" → "What columns do I need?" → "What new columns should I add?"
If a pipeline feels confusing, split it into steps and inspect the intermediate outputs.
:::
...
```{r code-tidier-comp, error=TRUE}
SACTN %>% 
  filter(site = "Amanzimtoti")
```

::: {.callout-note appearance="simple"}
## Why the error is helpful
...
```{r code-tidier-logic-1, eval=FALSE}
SACTN %>% 
  filter(site == "Pollock Beach", month(date) == 12 | month(date) == 1)
```
```{r code-tidier-logic-1-ghost, echo=FALSE}
...
```{r code-tidier-arrange, eval=FALSE}
SACTN %>% 
  arrange(depth, temp)
```
```{r code-tidier-arrange-ghost, echo=FALSE}
...
```{r code-tidier-desc, eval=FALSE}
SACTN %>% 
  arrange(desc(temp))
```
```{r code-tidier-desc-ghost, echo=FALSE}
arrange(SACTN, desc(temp))[1:10,]
```

When you arrange data in this way, any rows with `NA` values move to the bottom of the dataframe. Keep that behaviour in mind because it is not always desirable.
...
```{r code-tidier-filter, eval=FALSE}
SACTN %>% 
  filter(site == "Humewood", year(date) == 1990)
```
```{r code-tidier-filter-ghost, echo=FALSE}
...
```{r code-tidier-filter-assign, eval=FALSE}
humewood_90s <- SACTN %>% 
  filter(site == "Humewood", year(date) %in% seq(1990, 1999, 1))
```

`filter()` also removes any rows in the filtering column that contain `NA` values. If you want to keep rows with missing values, include `is.na()` in the filtering expression. To illustrate this, let us filter the temperatures for the Port Nolloth data collected by the DEA that were at or below 11°C or were missing values. Each argument is placed on a separate line to keep the logic clear. R automatically indents the last line in this chunk to show that it is part of the same argument. I have also put the final bracket on its own line because that closing bracket is easy to forget.

```{r code-tidier-filter-na, eval=FALSE}
SACTN %>% 
...
```{r code-tidier-sect-1, eval=FALSE}
...
```{r code-tidier-select-2, eval=FALSE}
...
```{r code-tidier-mutate-1, eval=FALSE}
SACTN %>% 
  mutate(kelvin = temp + 273.15)
```
```{r code-tidier-mutate-1-ghost, echo=FALSE}
