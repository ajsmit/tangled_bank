...
execute:
  ...
  fig-format: png
params: 
  # AJ renders Tasks with hide_answers: false in the Tasks YAML.
  # The wrappers below still allow hiding when needed (e.g., for a student version).
  hide_answers: false
---
...
```{r tbl-taskD-q5-gt, echo=TRUE, eval=TRUE}
...
Using the SACTN monthly data (`SACTNmonthly_v4.0.RData`), create a dataset that contains the **mean temperature** per `site` and `depth` (ignore `NA`s). 

Because there are many sites, **restrict your plot to the top 8 sites** (by number of observations in the raw `SACTN` table). Then plot mean temperature vs depth, with points and lines coloured by `site`. **(/12)**

`r if (params$hide_answers) "::: {.content-hidden}"`

**Answer**

```{r fig-taskD-q6, echo=TRUE, eval=TRUE, fig.width=7, fig.height=4.2}
#| fig-cap: "Mean SACTN temperature by site and depth (top 8 sites by data availability)."

load(here::here("data", "BCB744", "SACTNmonthly_v4.0.RData"))
SACTN <- SACTNmonthly_v4.0
rm(SACTNmonthly_v4.0)

# Pick a manageable number of sites for a legible legend
sites_top8 <- SACTN |>
  filter(!is.na(temp)) |>
...
SACTN_site_depth <- SACTN |>
  filter(site %in% sites_top8) |>
  group_by(site, depth) |>
  summarise(mean_temp = mean(temp, na.rm = TRUE), .groups = "drop")

ggplot(SACTN_site_depth, aes(x = depth, y = mean_temp, colour = site, group = site)) +
  geom_line(alpha = 0.8, linewidth = 0.5) +
  geom_point(alpha = 0.9, size = 1.6) +
  labs(x = "Depth (m)", y = "Mean temperature (°C)", colour = "Site") +
