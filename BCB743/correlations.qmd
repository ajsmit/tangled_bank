---
date: last-modified
title: "Environmental Correlations and Species Associations"
author: "A. J. Smit"
...
You met correlation as an inferential tool in [BCB744, Chapter 9](../BCB744/basic_stats/09-correlation-and-association.qmd), where a single coefficient summarised the association between one pair of variables at a time. Here we revisit the idea with environmental data and many variables at once. A correlation matrix for a river system is interpreted as an analysis of how one spatial gradient structures the whole dataset, and this is what later ordination and constrained methods build on.
...
The environmental table `env` holds 30 sites (rows) and 11 measured variables (columns). Reading the correlation matrix later is much easier once the variable codes are familiar, so we set them out here:
...
We do not need to standardise the variables, as one would for Euclidean distances, although in some instances a transformation might be needed:
...
::: {.callout-note appearance="simple"}
## Why We Should Assess Collinearity Here
...
The same redundancy is treated in detail in [BCB744, Chapter 16](../BCB744/basic_stats/16-collinearity-confounding-measurement-error.qmd), and its effect on fitted coefficients is shown in [BCB744, Chapter 14](../BCB744/basic_stats/14-multiple-regression-and-model-specification.qmd). It returns in this course when we reach constrained ordination, where variance inflation factors (VIF) are used to flag and prune collinear predictors.
...
To calculate associations *between species*, we first need to **transpose** the data, so that each row is a species and each column a site:

```{r code-spe-t-t-spe}
spe_t <- t(spe)
```

Now we can calculate the association matrix. The first version uses the abundances directly:
...
The fish matrix records the same longitudinal gradient as the environmental one, now expressed through which species share sites. Rather than scan all 27 species at once, we pull out six that span the river, from the cool upper reaches to the lowland sites, using the binary matrix:
...
The chapter has used four ideas that are easy to conflate, so let's separate them:
