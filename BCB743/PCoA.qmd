...
When `capscale()` is given a raw community table and a distance method, it can add species scores as weighted sums of the community matrix. If I instead supply only a precomputed dissimilarity matrix, that species information is no longer available from the input and the species scores are absent.
