...
<!-- The Option 3 approach might be useful if we want to calculate some dissimilarity matrix using a function that does not reside within the **vegan** universe (see, for example, [Handling mixed variable types](https://tangledbank.netlify.app/BCB743/PCoA.html#handling-mixed-variable-types)). -->
...
bioreg <- read.csv(here::here("data", "BCB743", "seaweed", "bioregions.csv"), header = TRUE)
