...
In such complex, high-dimensional data, analysing each variable separately using a series of univariate or bivariate analyses would be inefficient (if not impossible) and unlikely to reveal the underlying patterns accurately. For example, in the Doubs River dataset, a univariate approach would require (27 × 26) / 2 = 351 separate species-pair analyses, which is impractical and prone to misinterpretation.

The count is the smaller problem. Even with unlimited patience for 351 correlations, the pairwise approach would still miss what matters most, namely the structure that emerges only when all species and sites are considered together. A gradient that orders the whole community, or a group of sites that share a fauna, is a property of the joint configuration, not of any single pair. Ordination recovers that joint structure, so it does more than simply saving time.
...
| nMDS   | unconstrained | any distance matrix            | rank order           | complex communities, robust to outliers  |
