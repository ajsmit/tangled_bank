---
date: last-modified
title: "Distance-Based Redundancy Analysis: Revisiting the β-Diversity Partition"
author: "A. J. Smit"
...
Load the seaweed species data ($Y$), 58 coastal sections × 847 macroalgal species recorded as presence–absence [@smit2017seaweeds].
...
But the parts that tell us about the *ecology* do not change. The same four thermal terms stay significant, and once the two ordinations are optimally rotated onto one another their site configurations are almost coincident (Procrustes correlation `r round(pro$t0, 3)`, $p =$ `r format.pval(pro$signif, eps = 0.001)`). **The ecological conclusion holds under both partitions, even though the ordination is not numerically identical.** So, switching to Podani's cheme changes some numbers but not the answer, and had it changed the answer, that is something I would have wanted to know.
...
- **Check that the conclusion rwmains unchanged.** Run both, then compare with a Mantel test on the matrices and a Procrustes test on the ordinations, as above. When the answer is stable, say so. When it is not, the partition has become part of your result and must be reported as such.
:::

## Summary

The first version of this chapter was not wrong so much as it rested on a contested choice that it never flagged. Here I made the choice explicit, set out the critique of the Baselga turnover / nestedness-resultant split [@almeidaneto2012rethinking; @podani2011new; @schmera2011comments], adopted the additive, commensurably scaled replacement component of @legendre2014interpreting, and re-ran the db-RDA. The thermal structuring of South African seaweed dissimilarity ramins invariant of the change. The partition of β-diversity is a modelling decision, to be made deliberately, reported, and checked against the conclusion.

The broadest lesson is the one to carry into your own work. **An ecological conclusion worth trusting is one that is robust acorss reasonable analytical alternatives.** A result that holds only under a single, contested choice is fragile, whether or not anyone notices. A result that you have tried to break and could not is one you can defend. 
