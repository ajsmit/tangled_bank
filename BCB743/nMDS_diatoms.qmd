...
PERMANOVA asks whether the communities differ overall. It cannot say which taxa are responsible. `manyglm()` fits a separate generalised linear model to each diatom species and tests them together, so it identifies the individual taxa that drive a community-level difference. The two methods are complementary: first the community question, then the species question.
:::
