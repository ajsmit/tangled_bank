...
<about-the-seaweed-data>
In my examples (#link("deep_dive.qmd")[Gradients Example];, #link("https://tangledbank.netlify.app/docs/The-Biostatistics-Book-Ch-3-5.pdf")[Multiple Regression] and #link("constrained_ordination.qmd")[db-RDA];), I employ environmental variables derived from an extensive daily time series of seawater temperature. These data were transformed into various statistics that capture components of the thermal regime along the South African coastline:
...
+ Where such data exist, consider incorporating #strong[trait-based approaches] into your analysis. Functional traits of seaweeds (for example, thallus morphology, photosynthetic pigments, or reproductive strategies) may respond differentially to environmental gradients. This approach can illuminate mechanisms driving community composition beyond simple species presence/absence data. While I have not yet explored this avenue, I anticipate pursuing it as a future research project, probably through #strong[Fourth-corner analysis] or #strong[RLQ ordination];.
...
- R: #strong[Site-by-species abundance matrix] (the species table with the community data)
- L: #strong[Site-by-environment matrix] (the temperature variables, distance, bioregions, #emph[etc.];, in the environmental table)
- Q: #strong[Species-by-traits matrix] (morphological, physiological, or life-history characteristics)
...
body: 
[
RLQ ordination extends the fourth-corner concept into multivariate space and provides an integrated view of trait-environment relationships. It does a double ordination that maximises the covariance between environmental variables (table #strong[R × L];) and species traits (table #strong[L × Q];), with the species composition table (#strong[L];) bringing in the new concept.

Mathematically, RLQ finds linear combinations of environmental variables and linear combinations of traits that produce maximum correlation when projected through the species composition data. It reveals the main gradients along which traits and environmental conditions co-vary. The technique essentially performs a #strong[co-inertia analysis] between two indirect ordinations:
...
+ The #strong[annual mean temperature (`annMean`)] integrates data from warm and cold seasons to serve as an integrated predictor of global ecosystems. Regionally, it may function as a significant driver due to coastal temperature gradients, though potential collinearity with other variables requires examination.

+ The #strong[mean temperature of the warmest month (`febMean`)] displays a clear gradient from the east coast to Cape Point, but remains relatively stable along the west coast. Variability in this region is captured by `febSD`.

+ The #strong[temperature range of the warmest month (`febRange`)] differentiates the Benguela Current from the Agulhas Current. It exhibits both east-west and north-south gradients.

+ Looking at #strong[temperature variability] such as during the coldest and warmest monsts, `augSD` and `febSD`, respectively, give geographically-linked explanations, maybe showing how to upwelling intensity or current stability affect species structural patterns.

+ Employ #strong[unconstrained ordinations] with environmental vectors (`envfit()`) to guide selection of important structuring predictors.
...
+ Conduct #strong[multiple linear regressions] using continuous predictor variables as functions of dist (distance between site pairs).

+ Create #strong[thematic maps] (spatially implicit) of temperature variables and vary symbol size or colour intensity by magnitude. Maybe use GIS tools to interpolate values between sampling points for more comprehensive visualisation.

+ Assess #strong[spatial autocorrelation] in your variables using techniques such as Moran's I or Geary's C. This helps identify the scale at which environmental factors operate and informs model structure.
...
+ Select variables based on #strong[ecological understanding] of species' responses to environmental drivers, considering both direct and indirect effects.

+ Choose variables that reflect #strong[significant known environmental gradients] influenced by factors such as ocean currents, coastal topography, and climate patterns.

+ Use #strong[unconstrained ordinations] (#emph[e.g.];, Principal Component Analysis, PCA) to explore the relationships between environmental variables and species composition. This can help identify key gradients and inform variable selection. Running such ordinations side-by-side on the species and environmental data can reveal how well the environmental variables explain the observed species patterns. This method becomes especially useful when you superimpose the environmental vectors onto the ordination plot using `envfit()` or a GAM or something similar.

+ Also use #strong[data-driven decision making];. Employ statistical methods such as Variance Inflation Factors (VIFs) or forward selection (#emph[e.g.];, `stepAIC()`) to address multicollinearity and refine model selection. Consider modern techniques like elastic net regression or random forests for variable importance ranking.
...
+ Consider also spatial autocorrelation in the models. Beyond simply identifying spatial autocorrelation, one might develop explicit strategies for incorporating it into models. One such is #strong[Moran's Eigenvector Maps (MEMs)];, which can be used to account for spatial structure in species-environment relationships. MEMs are derived from distance matrices and can be included as predictors in regression models to capture spatial patterns.
]
...
title: 
[
#strong[Moran's Eigenvector Maps (MEMs)]
]
, 
background_color: 
rgb("#ccf1e3")
...
#block[
#set enum(numbering: "1.", start: 6)
+ Explore #strong[model averaging] techniques, such as Akaike weights or Bayesian Model Averaging, to account for model uncertainty. Model averaging yields more believable predictions and insights into the relative importance of different environmental variables.
]
...
+ #strong[Synthesise] theoretical insights on environmental gradients and biological responses with statistical techniques to build defensible models.

+ Develop and #strong[test hypotheses] using multiple regression models and consider both ecological relevance and statistical fit. Remain open to unexpected results that may challenge existing ideas.

+ Extend analysis to #strong[multivariate methods] (for example constrained ordinations for gradient detection and attribution, and clustering for group identification) to uncover more complex ecological patterns.
...
+ Consider modern techniques like #strong[Joint Species Distribution Models (JSDMs)] to simultaneously model multiple species and environmental factors.
]
...
The #strong[Hmsc] R package provides hierarchical JSDMs that can handle various response types and spatial/temporal structures.

]
, 
title: 
[
#strong[Joint Species Distribution Models (JSDMs)]
]
, 
background_color: 
rgb("#ccf1e3")
...
#block[
#set enum(numbering: "1.", start: 5)
+ Begin identifying the most influential species using approaches such as #strong[multivariate abundance analysis with Generalised Linear Models (GLMs)];, which offers a different approach to "Model-based Multivariate Analyses".
]
...
#strong[#emph[Uncertainty Quantification];]
...
#strong[#emph[Residual Analysis for Model Diagnostics];]
...
#strong[#emph[Reproducible Workflows];]
