...
Here $\theta$ is large and the negative binomial AIC barely improves on the Poisson, confirming that the overdispersion is mild and the conclusions do not change under that alternative. That is a good outcome which you can only establish it by checking it specifically. When the dispersion ratio is large (say above 2) and $\theta$ is small, the negative binomial standard errors are substantially wider than the Poisson ones, and reporting the Poisson model would be an error.
