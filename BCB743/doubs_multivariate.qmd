...
The unconstrained methods describe the pattern, so with the constrained methods I test whether the measured environment can explain it. I fit all three constrained ordinations, RDA on Hellinger-transformed abundances, CCA on the raw table, and db-RDA on Bray-Curtis dissimilarity, and in each case I let `ordiR2step()` select variables by forward selection from the null model up to the full model. You've seen this approach in the [constrained ordination chapter](constrained_ordination.qmd). Here I run the three in parallel and compare what they keep.
...
Sites that are far downstream are also far along the channel, so environment and space are structurally confounded. Variation partitioning (`varpart()`, which you've seen in the [seaweed appendix](two_oceans_appendices.qmd)) splits the community variation into a part explained uniquely by the environment, a part explained uniquely by spatial position, a part they share, and an unexplained remainder. I describe spatial position with a second-order polynomial of the site coordinates.
...
The preceding analyses describe the river as a long gradient interrupted by a short oxygen sag. A simple cluster analysis lets me turn that interpretation into an explicit reach partition. I use the three variables that the db-RDA selected, distance from source, oxygen, and BOD, because together they represent position along the river and the local pollution signal. I standardise them, compute Euclidean distances among sites, and use complete-linkage hierarchical clustering. This is not an independent causal test. It is a diagnostic grouping of sites under the same variables that explained the fish composition.
...
Every distance-based method treats the count table through a dissimilarity index. `manyGLM()` in **mvabund**, which you've seen in the [diatom nMDS chapter](nMDS_diatoms.qmd), instead fits a negative-binomial GLM to each species and tests the predictors jointly by resampling, so it respects the mean-variance relationship of counts explicitly. It is a useful check that the ordination result does not depend on the Bray-Curtis geometry.
