...
The two-dimensional configurations in @fig-unconstrained show what those
first-axis summaries compress. Sites are coloured by their distance from the
source, so a smooth colour transition across an ordination indicates recovery
of the longitudinal river gradient. DCA is included here because its axis
length is used below to diagnose the long species-turnover gradient, even
though it is not part of the four-method correlation comparison.

```{r fig-unconstrained}
#| label: fig-unconstrained
#| fig-cap: "Site configurations from each unconstrained ordination. Numbers identify sampling sites and colour shows distance from the source. PCA uses all 30 sites; the species ordinations exclude fishless site 8."
#| fig-width: 8
#| fig-height: 10.8
#| out-width: "100%"
#| code-fold: true
...
The selected ordinations are shown in @fig-constrained. Here the arrows are the
predictors retained by forward selection, and the sites are again coloured by
distance from source. Arrow direction shows increasing values; arrow length is
scaled within each panel for legibility and should not be compared across
methods.

```{r fig-constrained}
#| label: fig-constrained
#| fig-cap: "Selected RDA, CCA, and Bray-Curtis db-RDA ordinations. Points are sampling sites, colour indicates distance from the source, and arrows show the environmental variables retained by forward selection."
#| fig-width: 9
#| fig-height: 3.7
#| out-width: "100%"
#| code-fold: true
...
@fig-mem-ordination displays the spatial RDA itself. The arrows show the
selected spatial eigenvectors, while the colour gradient reveals whether their
combined configuration represents the broad river continuum or finer-scale
departures from it.

```{r fig-mem-ordination}
#| label: fig-mem-ordination
#| fig-cap: "RDA of fish composition constrained by the forward-selected Moran eigenvector maps. Points are sampling sites, colour indicates distance from the source, and arrows show the selected spatial eigenvectors."
#| fig-width: 5.5
#| fig-height: 4.2
#| code-fold: true
