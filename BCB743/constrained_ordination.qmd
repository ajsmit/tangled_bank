...
* **CCA** is the extension of a CA with multiple regression, and is therefore also based on 𝝌2-metric (dissimilarities). We do not have a choice of specifying which dissimilarity metric to use. CCA performs best when species distribution follows a unimodal model. Use **vegan**'s `cca()` to perform a CCA.
...
In this chapter we work entirely with **db-RDA**, for the same reason we favoured PCoA among the unconstrained methods: it accepts any ecological dissimilarity, which is what community data usually demand, and it follows directly from the distance-based thinking of the previous chapters. The path is the one we have walked before, with a second matrix added at the end (@fig-constrained-concept).
...
Both come from the same fitted model; we extract all the scores once and then plot each in turn. First the **site (wa) scores**:
