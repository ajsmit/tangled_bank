...
Ecological model building is not a mechanical search for the highest $R^2$. It requires biological judgement, a clear hypothesis, and statistics that match the structure of the data.

This page uses South African seaweed $\beta$-diversity as the working example, but the logic is general: decide what process you think matters, choose predictors that can represent that process, inspect their structure, and then build models you can actually defend.
...
I have also included geographical distance between site pairs along the coast. This creates an immediate conceptual problem: is distance a genuine predictor, or is it only a spatial summary of other underlying processes? The same caution applies to `bio`, the bioregional classification.

## Theoretical Understanding of Environmental Drivers

Begin by asking how each candidate variable could plausibly affect seaweed community structure. Temperature is biologically meaningful, but different summaries of temperature need not be equally meaningful. Means, ranges, and variability can imply different mechanisms, so you should not include them merely because they are available.
...
Environmental variables with strong spatial gradients are good candidates for environmental filtering, but that still needs to be demonstrated rather than assumed. To quantify these gradients:
...
Throughout model building, combine ecological reasoning with statistical diagnostics:
...
### Hierarchical and Multi-Scale modelling
