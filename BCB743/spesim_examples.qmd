...
ref <- landscape("systematic") # the shared community, sampled on a grid
```

```{r fig-ex6-fields}
#| fig-cap: "The three environmental fields of Example 6. Elevation is a radial hill, temperature a plane that warms eastwards, and rainfall an orographic crescent that wraps the hill, wettest on the south-west slopes and driest in the north-east. White lines are contours; a small autocorrelated residual is added to each."
#| fig-format: png
#| fig-dpi: 150
#| fig-width: 9.2
#| fig-height: 3.2
#| out-width: "100%"
env <- ref$env_gradients
...
round(RsquareAdj(dbrda)$adj.r.squared, 3)
```

```{r fig-ex6-biplot}
#| fig-cap: "db-RDA of the systematic sample, constrained by the three gradients. Arrows are the fitted environmental vectors; points are quadrats coloured by their elevation. The three arrows point in distinct directions, so each gradient leaves a separable imprint on the community, and the elevation arrow aligns with the colour trend along the first axis."
#| fig-width: 5.2
#| fig-height: 4.0
#| out-width: "72%"
sites <- as.data.frame(scores(dbrda, display = "sites", scaling = 2))
...
comparison6 <- bind_rows(lapply(runs6, `[[`, "metrics"))
```
...
if (has_gt) {
  comparison6 |>
    gt::gt() |>
    gt::cols_label(design = "Sampling design", realised = "Realised") |>
    gt::tab_spanner(
      label = "envfit r² (gradient recovery)",
      columns = c(elevation, temperature, rainfall)
    ) |>
    gt::tab_header(
      title = "One three-gradient community, five sampling designs"
    )
} else {
  knitr::kable(comparison6)
}
```

```{r fig-ex6-designs}
#| fig-cap: "Each design's quadrats over the rainfall field. The two transects place the same 20 quadrats but on different lines: the west-to-east transect crosses the wet-to-dry contrast, while the south-to-north transect runs along it, at nearly constant rainfall."
#| fig-width: 8.6
#| fig-height: 6.0
#| out-width: "100%"
