...
The block is long, but only a few numbers has the message you need. Find the **Proportion Explained** and **Cumulative Proportion** rows near the top and read across the first two columns:

- PC1 explains about 54% of the variation.
- PC2 explains about 20%.
- Together they account for about 74%.

Everything else (the species and site scores, the scaling note) is detail we extract separately below. For deciding how far the ordination compresses the data, those three percentages are the whole story.
:::
...
The number of axes to retain is a difficult question to answer. The first few axes will always explain the most variation, but how do we know how many reduced axes are influential and should be kept? Commonly recommended is the **broken stick** method, namely keep the principal components whose eigenvalues are higher than corresponding random broken stick components:
...
<!-- Now I will construct some primitive graphs of the Site and Species Scores to demonstrate how to interpret the eigenvectors associated with each eigenvalue. We will typically not do this kind of graphical display --- for plots suitable for publication see the [Biplots](PCA_examples.qmd#biplots) section, below. -->
...
The choice of scaling in a PCA biplot helps us to make the right inferences about our data. Each scaling answers a different question, and the interpretation of distances, angles, and vector lengths depends on which question is being asked. The table is the short version to memorise:
...
Now we create biplots using the `cleanplot.pca()` function that comes with the *Numerical Ecology with R* book. The figures are more or less the same, except that the scaling 1 plot adds a *circle of equilibrium contribution* (see *Numerical Ecology with R*, p. 125).

The circle has a precise meaning. If a variable contributed equally to every ordination axis, its arrow would have a predictable length in the two-dimensional plane we plot, namely the radius of the circle. For a plot of two axes built from $p$ variables, that radius is $\sqrt{2/p}$. A variable whose arrow reaches beyond the circle therefore contributes more than its equal share to the two displayed axes, and one whose arrow falls short contributes less. Two cautions follow. The circle is a reference length and not a significance test, so it tells us which variables are well represented in the plane on view, not which are statistically significant. It also applies only to scaling 1, where arrow lengths are directly comparable; under scaling 2 the arrows encode correlations rather than contributions, and the circle is not drawn. With those caveats, we read importance from the arrows that extend beyond the radius (@fig-pca-cleanplot):
...
Having run, plotted, and interpreted a PCA, we are in a position to see where it breaks down. On the Doubs environmental data PCA performed well, since the variables relate to the river gradient in a roughly linear way. On species data the picture often changes, and the most common symptom is the horseshoe.
...
- **Treating a PC axis as a measured variable.** An axis is a composite of many variables; it has no units and is interpreted through the loadings, not read off directly.
