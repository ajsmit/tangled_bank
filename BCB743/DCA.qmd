...
- **below *ca.* 3 SD**, the gradient is short, species responses are approximately monotonic over the sampled range, and **linear** methods (PCA, RDA) are appropriate;
...
- **between 3 and 4 SD**, the choice is not clear-cut, and it is worth fitting and comparing both families.
...
The dune meadows span about `r round(dune_len, 1)` SD and the Doubs fish about `r round(doubs_len, 1)` SD. Both exceed 3 SD and approach 4, which places them in the range where unimodal methods are preferred. That is consistent with the arch CA produced for each and with the unimodal species responses seen in the [Correspondence Analysis](CA.qmd) chapter; a short gradient (below 3 SD) would instead have justified a linear PCA or RDA on the raw data. This single number turns the choice of ordination from a matter of habit into a decision the data can inform.
