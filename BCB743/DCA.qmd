...
To see detrending on real data we switch, for this chapter only, from the Doubs River to the **dune** meadow dataset that ships with **vegan**. The dune data show the arch more clearly than the Doubs fish data, so the contrast between CA and DCA is easier to see; the principle is identical. The arch is plain in the CA panel of @fig-ca-dca, where the sites lie along a curved locus, and the DCA panel shows the same sites with the arch removed.
