...
- **Limitations:** It counts shared absences as evidence of similarity (the "double-zero problem"), and it is dominated by abundant species and by differences in scale, which is why it is not recommended for raw species abundance data unless the data are first transformed (for example with the Hellinger transformation).
