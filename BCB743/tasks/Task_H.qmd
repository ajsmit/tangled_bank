...
prop_constr <- dbrda$CCA$tot.chi / dbrda$tot.chi
c(adj_R2 = round(adjR, 3), prop_constrained = round(prop_constr, 3))
```

The whole-model permutation test is highly significant, so the environmental subset explains more of the fish composition than chance would. The adjusted $R^2$ is **`r round(adjR, 2)`** and the constrained inertia is **`r round(100 * prop_constr, 1)`%** of the total: a sizeable, real share of compositional variation is attributable to these measured gradients, with the rest left unconstrained (unmeasured drivers and noise).
:::
