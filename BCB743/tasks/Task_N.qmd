...
dat <- tibble(total = rowSums(pyrifos), dose, week, ditch)   # pyrifos is ln-transformed abundance

naive <- lm(total ~ dose, data = dat)                        # ignores ditch
...
#| code-fold: false
#| label: task-n-q2
mm <- lmer(total ~ dose + (1 | ditch), data = dat)
...
m_main <- lmer(total ~ dose + week_num + (1 | ditch), data = dat, REML = FALSE)
m_int  <- lmer(total ~ dose * week_num + (1 | ditch), data = dat, REML = FALSE)
anova(m_main, m_int)                           # is the dose-by-time interaction supported?

m_slope <- lmer(total ~ dose + week_num + (1 + week_num | ditch), data = dat)
