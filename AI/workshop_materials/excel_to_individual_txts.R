# Excel to Individual TXT Converter
# Converts each row of an Excel file into a separate TXT file with headers as markdown-style headings
#
# Usage:
#   source("excel_to_individual_txts.R")
#   convert_excel_to_individual_txts()
...
#' Convert Excel file to individual TXT files per row
#' @param input_file Path to the Excel file
#' @param output_dir Directory to save the individual TXT files
#' @param pii_columns Vector of column names to exclude for privacy
#' @param filename_prefix Prefix for the output TXT files (default: "Gansbaai_survey")
convert_excel_to_individual_txts <- function(
  ...
  for (i in 1:nrow(data)) {
    ...
    # Create filename
    filename <- paste0(filename_prefix, "_", row_number, ".txt")
  ...
  cat("Successfully created", nrow(data), "TXT files in", output_dir, "\n")
}
...
#' Clean up generated TXT files
#' @param output_dir Directory containing the TXT files to remove
clean_txt_files <- function(output_dir = OUTPUT_DIR) {
  if (!dir.exists(output_dir)) {
  ...
  if (length(txt_files) == 0) {
    cat("No TXT files found in:", output_dir, "\n")
    return()
  }

  cat("Removing", length(txt_files), "TXT files from", output_dir, "\n")
