'PCNM' <-
	function(
		...
		#    Distance-based MEM is described in Dray et al. 2006.
		...
		a <- system.time({
			...
			if (moran) {
				...
				weight.dist.coord.mat <- as.matrix(
					1 - (as.dist(matdist) / (4 * threshh))^2
				)
		...
		if (is.null(thresh)) {
			if (moran) {
				res <- list(
					values = mypcnm.all$values,
					vectors = mypcnm.all$vectors,
					Moran_I = Moran,
					expected_Moran = res$expected,
					spanning = spanning,
					thresh = threshh + 0.000001
				)
			} else {
				res <- list(
					values = mypcnm.all$values,
