...
\setlength{\tableblockwidth}{\dimexpr 34mm + 136mm + 20pt\relax}
\newlength{\tableoffset}
\setlength{\tableoffset}{10mm}
\newlength{\headerleftwidth}
\setlength{\headerleftwidth}{100mm}
\newlength{\headergap}
\setlength{\headergap}{7mm}
\newlength{\headerrightwidth}
\setlength{\headerrightwidth}{\dimexpr \tableblockwidth - \headerleftwidth - \headergap\relax}
\setlength{\LTpre}{0pt}
\setlength{\LTpost}{0pt}
\setlength{\LTleft}{\tableoffset}
...
\begin{minipage}[t]{\headerleftwidth}
  ...
  {\small Map of the course chapters to complement the statistics methods cheat sheet. It is meant for revision, orientation, and linking ideas rather than rapid method lookup.}
\end{minipage}%
\hspace*{\headergap}%
\begin{minipage}[t]{\headerrightwidth}
  ...
  \begin{summarybox}
    ...
    Use the statistics methods cheat sheet when you need concrete test choice, assumptions, reporting details, or R functions.
  \end{summarybox}
\end{minipage}
\end{minipage}
...
\begin{longtable}{>{\raggedright\arraybackslash}p{34mm} >{\raggedright\arraybackslash}p{\dimexpr \tableblockwidth - 34mm - 4\tabcolsep\relax}}
\toprule
\rowcolor{headbg}
...
\MapRow{Chapter 3}{Core}{Graphs are part of the analysis. Choose plots that reveal structure, spread, outliers, and grouping before fitting models.}
\MapRow{Chapter 4}{Core}{Probability distributions and sampling variation explain why estimates fluctuate. Standard errors and confidence intervals arise from repeated sampling.}
...
\MapRow{Chapter 6}{G}{Assumptions attach to the scale and design of the analysis, so check normality, equal variance, and transformation needs before committing to Gaussian procedures.}
...
\MapRow{Chapter 8}{G}{ANOVA is the multi-group extension of mean comparison. The global test asks whether any group means differ, and post hoc procedures locate the differences.}
...
\MapRow{Chapter 10}{Work}{Method selection follows data structure: response type, number of groups, predictor type, pairing, and independence determine the inferential methods.}
\MapRow{Chapter 11}{G}{Regression assumptions are checked on residuals because fitting removes the systematic mean structure. Therefore, use residual plots to guide model revision.}
\MapRow{Chapter 12}{G}{Simple linear regression models the conditional mean as a line. The slope inference, fitted values, and confidence versus prediction intervals answer different questions.}
...
\MapRow{Chapter 19}{Grp}{Mixed models represent grouped or repeated-measures data by combining fixed effects with random effects. Dependence must be modelled and not simply ignored.}
...
\MapRow{Chapter 22}{NL}{Nonlinear regression is appropriate when biology implies a specific nonlinear equation. Now, parameter meaning comes from the model form and biological theory. Starting values often matter.}
\MapRow{Chapter 23}{Q}{Quantile regression models conditional quantiles rather than only the conditional mean. It reveals how effects differ across the response distribution.}
...
\MapRow{Chapter 26}{Work}{Reproducible workflow is part of statistical practice, so script the analysis, record decisions, manage outputs systematically, and keep the inferential path inspectable.}
\end{longtable}
}
