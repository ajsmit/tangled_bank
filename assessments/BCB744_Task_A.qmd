...
Use the result produced by `as.vector()` (your own data) and assemble three new arrays with a different combinations of dimensions. Show the dimensions and lengths of the new arrays.
...
Why am I pedantic about using commas and periods correctly in my code? Name some use cases of commas and periods.
...
Create a script to read in the file `crops.xlsx` and assign its content to the object `crops`.
...
Save the newly-created object to a CSV file called `crops2.csv` within your workspace.
...
What purpose can the naming of a newly-created dataframe serve? Name at least five reasons.
...
Using annotated R code, demonstrate your understanding of the various ways to look inside of the `crops` object. Show at least five different ways to inspect the dataframe.
...
Explain what you see inside the file. What are the columns? What are the rows? What are the data types?
...
Explain in words what the pipe operator `%>%` does in R. How does it make your code more readable?
...
Using the various **tidyverse** functions, calculate the mean ± SD for the crop mass within each combination of `block` and `fertiliser` of the `crops` dataset.
