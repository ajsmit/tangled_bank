...
- The seaweed environmental data -- [`SeaweedEnv.csv`](../data/BCB743/seaweed/SeaweedEnv.csv)
- The seaweed coastal sections -- [`SeaweedSites.csv`](../data/BCB743/seaweed/SeaweedSites.csv)
...
```{r code-spp-richness-tibble-section}
spp_richness <- tibble(
    section = 1:58,
...
```{r code-round-mean-spp-richness-richness}
...
```{r code-light-read-csv-here}
light <- read.csv(here::here("data", "BDC334", "light_levels.csv"))
light
```
...
```{r code-h-diversity-light-margin}
...
ncol(spp) # <1>`
```
1. The number of columns counts all species listed in the file, including any with no recorded presences.

We can also use the `specnumber()` function to calculate $\gamma$-diversity by applying it to columns where the `sum > 0`:

```{r code-richness-pooled-specnumber-colsums-spp, eval=TRUE}
...
**Total: 14 marks for BDC334 students.**

1. **(/ 2)** Why is there a difference between the two?
...
## Model answers: Tasks 1--2

### 1. Why the totals differ

**Marking: 2 marks; 1 for both totals and 1 for the all-zero column.**

`ncol(spp)` returns **847**, whereas `specnumber(colSums(spp > 0))`
returns **846**. The file lists 847 species, but the column `ANTNEM`
contains only zeros across all 58 sections. It contributes to the
column count without contributing a recorded species to the pooled
assemblage. We can check this directly:

```{r lab3-answer-gamma-check}
c(
    listed_species = ncol(spp),
    recorded_species = specnumber(colSums(spp > 0))
)
names(spp)[colSums(spp > 0) == 0]
```

### 2. The correct regional richness

**Marking: 1 mark for the pooled presence-based count of 846.**

For the observed $\gamma$-diversity of these 58 sections, **846 species**
is correct. A species must occur in at least one section to enter the
regional count. `sum(colSums(spp > 0) > 0)` gives the same result.
Counting columns works only when every species column has at least one
presence. This is richness recorded in the supplied dataset, not an
estimate of all seaweed species that might occur along the coast.
:::
...
Whittaker's multiplicative $\beta$-diversity is regional $\gamma$-diversity divided by mean local $\alpha$-diversity [@whittaker1960vegetation]. Here, `richness_pooled / mean(spp_richness$richness)` gives that regional summary. To see how each section's richness compares with the regional pool, we can also plot $\gamma / \alpha_i$ for each section (@fig-true-beta). These section-specific ratios are not separate estimates of regional $\beta$-diversity.

```{r fig-true-beta}
#| fig-cap: "Regional species richness divided by the richness of each coastal section."
true_beta <- data.frame(
    beta = richness_pooled / specnumber(spp, MARGIN = 1),
    section_no = c(1:58)
)
# true_beta
...
$$
\beta_\text{sør} = \frac{b + c}{2a + b + c} = 1 - \frac{2a}{2a + b + c}.
$$
...
```{r code-sor-vegdist-spp-binary}
sor <- vegdist(spp, method = "bray", binary = TRUE) # Sørensen dissimilarity
...
```{r code-sor-df-data-frame-round}
...
3. **(/ 2)** Why is the matrix square, and what determines the number of rows/columns?
4. **(/ 1)** What is the meaning of the diagonal?
5. **(/ 2)** What is the meaning of the non-diagonal elements?
6. **(/ 3)** Referring to the seaweed species data specifically, take the data in row 1 or column 1 and create a line graph showing these values as a function of the section number.
...
::: {.callout-tip appearance="simple"}
## Model answers: Tasks 3--7

### 3. Why the matrix is square

**Marking: 2 marks; 1 for the same sites on both axes and 1 for 58 × 58.**

Each of the 58 coastal sections is compared with every section,
including itself. Rows and columns therefore identify the same 58
sites, giving a **58 × 58** matrix. The 847 species columns in `spp`
supply the information for these comparisons; they do not determine
the dimensions of the site-by-site matrix. Its two triangles repeat
the same comparisons, so there are $58 \times 57 / 2 = 1653$ unique
off-diagonal pairs.

### 4. The diagonal

**Marking: 1 mark for zero dissimilarity in self-comparisons.**

The entry at row $i$, column $i$ compares Section $i$ with itself. Its
species composition is identical to itself, so dissimilarity is **0**.
This says nothing about how many species occur there: a species-rich
section and a species-poor section both have a zero on the diagonal.

### 5. The non-diagonal entries

**Marking: 2 marks; 1 for pairwise compositional differences and 1 for
both Sørensen endpoints.**

The entry at row $i$, column $j$ measures the difference in species
composition between Sections $i$ and $j$. In this presence--absence
Sørensen matrix, **0** means identical sets of recorded species and
**1** means no species in common. Intermediate values indicate partial
overlap. For example, Sections 1 and 2 have dissimilarity about **0.0036**,
whereas Sections 1 and 58 have dissimilarity about **0.9559**. This is a
dimensionless index, not a distance in kilometres or a difference in
species richness alone. Other indices available in `vegdist()` may
have different ranges; the upper limit of 1 is specific to the indices
for which it is defined.

### 6. Dissimilarity relative to Section 1

**Marking: 3 marks; 1 for code, 1 for all 58 correct plotted values and
1 for readable, correctly labelled axes.**

Extract one column of the full matrix. Every value then compares a
section with the same reference, **Section 1**. Extracting row 1 gives
the same values because the matrix is symmetric. Use the unrounded
matrix for calculations and plotting:

```{r fig-lab3-answer-sorensen}
#| fig-cap: "Sørensen dissimilarity between Section 1 and each of the 58 coastal sections. All comparisons use the same reference section."
sor_matrix <- as.matrix(sor)
sor_section1 <- tibble(
    section = seq_len(nrow(spp)),
    sorensen = sor_matrix[, 1]
)

ggplot(sor_section1, aes(x = section, y = sorensen)) +
    geom_line(linewidth = 1, colour = "indianred") +
    labs(
        x = "Coastal section, west to east",
        y = "Sørensen dissimilarity from Section 1"
    ) +
    scale_y_continuous(limits = c(0, 1)) +
    theme_linedraw()
```

Plotting row 1 or column 1 of the rounded `sor_df` is also acceptable.

The curve begins at **0**, remains low in the western sections and
generally rises towards the east, reaching about **0.956** at Section
58. It is not strictly increasing: for example, it falls from about
0.225 at Section 13 to 0.074 at Section 14. This graph shows comparisons
with Section 1, not dissimilarities between successive sections.

### 7. A possible ecological mechanism

**No marks are assigned to this question.**

One plausible explanation is **environmental filtering**, a niche-based
assembly process. Along the coast's thermal gradient, species with
different temperature tolerances can survive, grow and reproduce in
different sections. Assemblages then become less similar to the
western reference as conditions change eastwards. This interpretation
is consistent with the analysis of @smit2017seaweeds.

The curve alone does not establish that mechanism. Geographic and
environmental separation covary, and dispersal limitation and historical
barriers could also contribute. Total Sørensen dissimilarity combines
replacement and nestedness-resultant differences. Distinguishing these
explanations requires the partition below and analyses of environmental
and spatial predictors, rather than inferring a process from the curve
alone.
:::
...
8. **(/ 3)** Plot species turnover as a function of Section number, and provide a mechanistic explanation for the pattern observed. Use Section 1 as the reference, as in Question 6.
...
::: {.callout-tip appearance="simple"}
## Model answers: Tasks 8--9

### 8. Species turnover relative to Section 1

**Marking: 3 marks; 1 for code, 1 for all 58 correct plotted values and
1 for readable, correctly labelled axes. The mechanistic explanation
is not marked.**

Use `Y.pair$beta.sim`, the turnover component, rather than total
Sørensen dissimilarity or species richness. Column 1 of `Y1` gives the
same comparisons rounded to three decimal places and is also acceptable.

```{r fig-lab3-answer-turnover}
#| fig-cap: "The turnover component of Sørensen dissimilarity between Section 1 and each coastal section. The vertical axis measures species replacement, not species richness."
turnover_matrix <- as.matrix(Y.pair$beta.sim)
turnover_section1 <- tibble(
    section = seq_len(nrow(spp)),
    turnover = turnover_matrix[, 1]
)

ggplot(turnover_section1, aes(x = section, y = turnover)) +
    geom_line(linewidth = 1, colour = "indianred") +
    labs(
        x = "Coastal section, west to east",
        y = "Turnover (beta-sim) from Section 1"
    ) +
    scale_y_continuous(limits = c(0, 1)) +
    theme_linedraw()
```

Turnover is **0** for Sections 1--4 relative to Section 1, remains low
through the western sections and rises eastwards to about **0.928** at
Section 58. It never exceeds total Sørensen dissimilarity for the same
pair. At Section 58, total dissimilarity is approximately 0.956,
partitioned into 0.928 turnover and 0.028 nestedness-resultant
dissimilarity.

An ecological explanation is that changing thermal conditions favour
different species, so western species are progressively replaced by
species suited to other conditions. This is consistent with environmental
filtering. Turnover describes the compositional pattern, however; it
does not by itself prove niche-based assembly or exclude dispersal
limitation and historical effects.

### 9. Nestedness-resultant beta-diversity

**No marks are assigned to this question.**

Nestedness describes a pattern in which a species-poor assemblage is a
subset of a richer assemblage. @baselga2010partitioning separates the
portion of Sørensen dissimilarity attributable to this pattern from
species replacement:

$$
\beta_{\mathrm{sor}} = \beta_{\mathrm{sim}} + \beta_{\mathrm{sne}},
\qquad
\beta_{\mathrm{sne}} = \beta_{\mathrm{sor}} - \beta_{\mathrm{sim}}.
$$

For example, suppose one shore has species A, B and C, while another
has A, B, C, D and E. Then $a = 3$, $b = 0$ and $c = 2$.
Sørensen dissimilarity is $2/8 = 0.25$, turnover is 0, and
nestedness-resultant dissimilarity is 0.25. The poorer assemblage is a
complete subset of the richer one. Unequal richness alone does not
establish nestedness because sites can also contain different species.
The nestedness-resultant component quantifies a contribution to
dissimilarity, rather than serving as a general index of nestedness
for the whole species matrix. See the
[partitioning framework](https://doi.org/10.1111/j.1466-8238.2009.00490.x).

For a marine example, @smit2017seaweeds found a greater role for
nestedness-resultant differences along South Africa's west coast,
whereas turnover was more prominent along the south and east coasts.
They proposed historical restrictions on connectivity around the Cape
Peninsula as an explanation for this contrast. Their
[seaweed study](https://www.frontiersin.org/journals/marine-science/articles/10.3389/fmars.2017.00404/full)
illustrates why total dissimilarity alone can conceal different
compositional patterns.

Selective colonisation or loss can produce subsets, but a positive
$\beta_{\mathrm{sne}}$ does not identify which mechanism operated.
Environmental filtering can also produce nested subsets when only
tolerant species persist under restrictive conditions. Nestedness is
therefore not proof of neutral assembly, and a spatial comparison alone
does not demonstrate that species became locally extinct over time.
:::
...
The Lab 3 assignment is due at **08:00 on Monday, 21 September 2026**.
...
The marked components total **14 marks**. Questions 7 and 9 and the
mechanistic explanation in Question 8 remain learning exercises without
marks. Figure presentation is assessed through the explicit axis and
readability criteria in Questions 6 and 8; there is no separate
formatting mark.
:::
