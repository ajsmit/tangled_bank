...
subtitle: "Continuing to look at environmental data"
---
...
-   Example env data -- [`Euclidian_distance_demo_data_env.csv`](../data/Euclidian_distance_demo_data_env.csv)
...
The toy data have arbitrary columns to demonstrate the Euclidian
...
Calculate the Euclidian distance using **vegan**'s `vegdist()` function
...
These are the same data as in `Euclidian_distance_demo_data_xyz.csv` but
...
3. Using R, calculate the Euclidian distances for these data and display a portion of the resultant distance matrix.

4. Discuss the ecological conclusions you are able to draw from these Euclidian distances. Provide a few graphs to substantiate your answer.
:::
...
When we calculate Euclidian distances between geographic lat/lon
...
```{r}
#| fig-cap: "Line plots showing the relationship between Euclidian and geographical distance."
#| label: fig-geodist
#| fig-align: center
#| fig-width: 8
#| fig-height: 2.75
...
8.  Do a full analysis of the Doubs River environmental data using
    Euclidian distances and correlations. Demonstrate graphically any
...
and is due at **07:00 on Monday 15 August 2022**.\|
