...
#### Calculating a SAD from a Site x Species Table

Here’s how to manually derive an SAD from a typical community dataset. Let's assume you have a site x species table, where rows are sampling sites and columns are species, with the cells containing counts (abundances).

1. **Get Total Species Abundances**: Your first step is to get the total abundance for each species across all your sites. You do this by **summing each column**. This collapses your site x species matrix into a single list of numberswhich we call an "**abundance vector**;" here each number is the total count for one species.
...
3. **Plot the SAD**: This frequency table is your SAD. To visualize it as a frequency distribution (like for Fisher's or Preston's models), you plot the **Abundance Class on the *x*-axis** and the **Number of Species (frequency) on the *y*-axis**. You will typically see that classic hollow curve.
...
#### Calculate the OAR from a Site x Species Table

Calculating and plotting this relationship is easy. Let's start with your site x species table with cells containing abundance counts.
...
Calculating a proper SAC from a site x species table involves more than just adding up samples in their original order. The order in which you "collect" your samples can drastically change the shape of the curve. To solve this, the process relies on repeated randomisation to create a smooth, average curve.

1. **Randomise the Order of Samples**

The first and most important step is to randomise the order of your samples (the rows in your site x species table). This ensures the resulting curve isn't biased by happening to have a particularly species-rich or species-poor site at the beginning of the sequence.
...
This method, which is very similar to a species accumulation curve, estimates the expected number of species for a given number of samples (e.g., sites, quadrats). Here’s the recipe, which uses your entire site x species table:

1. **Define the Sample Pool**: Your pool is the complete set of samples (all the rows in your site x species table). Let's say you have $K$ total samples.
