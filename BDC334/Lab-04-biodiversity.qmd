...
## Lab goals and relation to Lecture 6 {#sec-lab04-goals}

This lab translates the analytical framework from [Lecture 6](Lec-06-unified-ecology.qmd) into executable analyses.

- Analytical framing and workflow rationale: see [Why "Unified Ecology"](Lec-06-unified-ecology.qmd#sec-lec06-why) and [Integrated Workflow](Lec-06-unified-ecology.qmd#sec-lec06-workflow).
- Pattern-family framework and assumptions: see [Core Pattern Families](Lec-06-unified-ecology.qmd#sec-lec06-patterns), [Hypothesis-Testing Frame](Lec-06-unified-ecology.qmd#sec-lec06-testing), and [Common Failure Modes](Lec-06-unified-ecology.qmd#sec-lec06-fail).

The practical flow in this lab is: **setup → pattern demonstrations → interpretation → exercises**.

## The Data {#sec-lab04-setup}
...
## Practical A. Species-Abundance Distribution {#sec-lab04-sad}

Theory link: see [Core Pattern Families](Lec-06-unified-ecology.qmd#sec-lec06-patterns) in [Lecture 6](Lec-06-unified-ecology.qmd).
...
The first formulation of this pattern was developed by @fisher1943relation, who observed that species frequencies often conform to what he termed a **logarithmic series distribution**. In this representation, *the expected number of species with a given number of individuals declines rapidly as abundance increases*, showing communities where most species occur at low frequencies and only a small fraction reach very high densities. The curve of Fisher’s log-series distribution thus shows the expected number of species $f$ with $n$ observed individuals, and the interpretation of this curve is structurally similar across all SAD models—only the underlying mathematics and rationale differ. Fisher’s model remains widely used as a theoretical benchmark and as a statistical model. In **R**, the **vegan** package implements this procedure through the `fisherfit()` function, which estimates the log-series parameter and compares it to observed community data (@fig-fishers-log).
...
BCI_OA <- data.frame(
  occ = apply(BCI, MARGIN = 2, count_fun),
  ab  = apply(BCI, MARGIN = 2, function(x) mean(x[x > 0]))
)
...
## Practical C. Species-Area (Accumulation) {#sec-lab04-accum}

Theory link: see [Alpha–Beta–Gamma linkage](Lec-06-unified-ecology.qmd#sec-lec06-abg) and [Integrated Workflow](Lec-06-unified-ecology.qmd#sec-lec06-workflow) in [Lecture 6](Lec-06-unified-ecology.qmd).


Species accumulation curves—often presented as sample-based species–area relationships (SAR)—address the question: given what has already been sampled, how many species are yet unseen, and how does observed richness grow as sampling effort expands? Within a single ecosystem type, one ordinarily expects richness to rise with the number of distinct sampling units (plots, quadrats, trawls), then bend toward a plateau as further additions yield few or no new species. The curve is thus a record of discovery under a specified sampling design. In this curve, the $x$-axis represents cumulative sampling effort (number of sites, or cumulative area if sites are areally commensurate), while the $y$-axis records the expected number of species discovered at that effort. As [Roeland Kindt](http://apps.worldagroforestry.org/downloads/Publications/PDFS/b13695.pdf) explains (see p. 41 in the cited note), the curve is generated by “adding” sites and tallying the average richness at each step—an averaging that already hints at a statistical question: in what order are sites added, and how is that order handled?
...
The final species accumulation curve is the average of all the randomised curves you generated.
...
## Practical D. Rarefaction Curves {#sec-lab04-rare}

Theory link: see [From Measures to Models](Lec-06-unified-ecology.qmd#sec-lec06-measures-models) and [Common Failure Modes](Lec-06-unified-ecology.qmd#sec-lec06-fail) in [Lecture 6](Lec-06-unified-ecology.qmd).
...
## Note: iNEXT is run here with incidence settings for an abundance-origin dataset.
## The function coerces data internally; interpret results as incidence-based outputs,
## not abundance-based estimates from this input form.
```
...
1. **Create the Individual Pool**: From your chosen site's row, create an explicit "pool" of all individuals. For example, if the site has 3 individuals of Species A, 0 of Species B, and 5 of Species C, your pool is a list containing `(A, A, A, C, C, C, C, C)`. The total number of individuals in this pool is $N$.
...
## Practical E. Distance-Decay Curves and Elevation (and Other) Gradients {#sec-lab04-dd}

Theory link: see [Distance-decay and ordination framework](Lec-06-unified-ecology.qmd#sec-lec06-ordination) and [Hypothesis-Testing Frame](Lec-06-unified-ecology.qmd#sec-lec06-testing) in [Lecture 6](Lec-06-unified-ecology.qmd).


The principles of distance-decay relationships are closely linked to $\beta$-diversity turnover concepts introduced in Lecture 4 (especially turnover components). Distance decay is the primary explanation for the spatial pattern of $\beta$-diversity along the South African coast in @smit2017seaweeds. A deeper dive into distance-decay calculation can be seen in [Deep Dive into Gradients](../BCB743/deep_dive.qmd).

In one sense, an elevation gradient can be seen as specific case of distance decay. The Doubs River dataset offers a nice example of data collected along an elevation gradient (which also happens to be a gradient along a river). Elevation gradients have many similarities with depth gradients (*e.g.*, down the ocean depths) and latitudinal gradients. 

::: callout-note
## Interpretation Checklist {#sec-lab04-interpret}
...
The Lab 4 assignment is due at **08:00 on Monday, 28 September 2026** and
will not be pre-assessed. It must nevertheless be uploaded to iKamva by the
stated deadline and completed as a self-assessed submission. Upload the
completed self-assessment to iKamva by 23:59 on the same day. See the
