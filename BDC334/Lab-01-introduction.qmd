...
@fig-euclidian-env shows how we may calculate Euclidean distance in MS Excel using some built-in functions. The function `SUMX2MY2` calculates the sum of the differences of squares between two corresponding arrays. It squares each value in array `x`, squares the corresponding value in array `y`, subtracts the `y`-square from the `x`-square, and then sums all these differences. That value is then subjected to a square-root calculation using `SQRT`.
