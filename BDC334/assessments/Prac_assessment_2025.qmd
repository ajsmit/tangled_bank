...
- [`BarentsFish.xls`](../../data/BCB743/BarentsFish.xls) (a Microsoft Excel file with the full dataset and a description of the content on the first sheet)
- [`BarentsFish_env.csv`](../../data/BCB743/BarentsFish_env.csv) (a CSV file with the environmental data)
- [`BarentsFish_spp.csv`](../../data/BCB743/BarentsFish_spp.csv) (a CSV file with the species data)
...
a. List the three pairs of sites that are furthest apart in terms of the **geographical distance** (*i.e.*, lat/lon needs to account for earth's curvature) between them. For each pair, also provide the temperature and depth associated with each member of the pair.
...
```{r code-library-vegan}
library(vegan)
library(geodist)
library(tidyverse)

env <- read.csv(here::here("data", "BCB743", "BarentsFish_env.csv"))
...
```{r code-geo-dist-mat-round-geodist-geo-dat}
geo_dist_mat <- round(geodist(geo_dat, paired = TRUE, measure = "geodesic"), 2)
```
...
```{r code-geo-dist-mat-upper-tri-geo-dist-mat}
geo_dist_mat[upper.tri(geo_dist_mat, diag = TRUE)] <- NA
```
...
```{r code-largest-dist-indices-order-geo-dist-mat}
largest_dist_indices <- order(geo_dist_mat,
    decreasing = TRUE, na.last = NA
)[1:3]

# retrieve the row and column indices of these largest distances
row_indices <- row(geo_dist_mat)[largest_dist_indices]
col_indices <- col(geo_dist_mat)[largest_dist_indices]
```
...
```{r code-largest-dist-pairs-data-frame}
...
```{r code-shortest-dist-indices-order-geo-dist-mat}
...
```{r code-env-dat-decostand-env-c}
# a. List the three pairs of sites that are furthest apart in terms of the
...
env_dat <- decostand(env[, c("Depth", "Temperature")],
    MARGIN = 2,
    method = "standardize"
)
...
Your code will bring you to the above step, and from there you can accomplish the rest manually to assemble the table by hand. I'll continue with more efficient code...

```{r code-env-dist-mat-as-matrix-env-dist}
# proceed as I did with the geodist calculations
env_dist_mat <- as.matrix(env_dist)

env_dist_mat[upper.tri(env_dist_mat, diag = TRUE)] <- NA
env_df <- as.data.frame(as.matrix(env_dist_mat))
env_df[1:7, 1:7]
```

```{r code-largest-dist-indices-order-env-dist-mat}
...
```{r code-shortest-dist-indices-order-env-dist-mat}
# b. List three pairs of sites that are closest together in terms of the
# **environmental distance** between them. For each pair, also state the
# environmental distance between them.
...
spp_dat <- read.csv(here::here("data", "BCB743", "BarentsFish_spp.csv"))
