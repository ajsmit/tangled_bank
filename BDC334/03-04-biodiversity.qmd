...
species at the small (local) scale, such as for example within a sampling unit like a quadrat, transect,
plot, or trawl. Or maybe the research question requires that the local scale be represented by a number of sampling units nesting within a small patch of landscape, and the mean species richness within this patch is defined as local. Multiples of these (sampling units, or patches) are nested within a larger region (or
...
logarithmic
series](03-04-biodiversity.html#species-abundance-distribution) (see functions `fisher.alpha()` and `fisherfit()`). The estimation is possible only for actual counts (i.e. integers) of individuals, so it will not work for percent cover, biomass, and other measures that can be expresses by real numbers. We will get to this function later under [Fisher's
logarithmic
series](03-04-biodiversity.html#species-abundance-distribution).

Except for Fisher's-$\alpha$, we cannot calculate these for the seaweed data because in order
...
seen in [Lab 1](01-introduction.qmd). In [Lab 2b](02b-env_dist.qmd) you will have also learned how to calculate these ecological distances in R.
...
Contemporary views of $\beta$-diversity are available by @nekola1999distance, @baselga2010partitioning, and @anderson2011navigating. Nowadays we see $\beta$-diversity is
...
-   Shade et al. (2018) [`Shade_et_al_2018.pdf`](../docs/Shade_et_al_2018.pdf)
:::
