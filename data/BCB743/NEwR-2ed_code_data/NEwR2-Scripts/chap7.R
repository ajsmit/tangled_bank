...
### WARNING ###
# The following code is the one proposed in the book (second edition).
# However, since its publication, the function at the core of the
# analysis, test.w {adespatial}, has been deprecated. It is still
# in function as per adespatial 0.3-14, so the code below still works.
# Interested users are invited to explore the new, more user-friendly
# functions listw.select, mem.select and associate functions to perform
# the analyses. A tutorial is available at the following address:
# https://cran.r-project.org/web/packages/adespatial/vignettes/tutorial.html#swm
...
### Attempts with the new functions.
...
# Delaunay triangulation and model selection
(mite.del <- tri2nb(mite.xy))
mite.delw <- nb2listw(mite.del)
mite.del.res2 <- 
  mem.select(mite.h.det, 
         mite.delw, 
         MEM.autocor = "positive")
# Summary of the results for the best model
summary(mite.del.res2$best)
# Unadjusted R^2 of the model with the smallest AICc value
(R2.del <- 
  mite.del.res2$best$AIC$R2[which.min(mite.del.res2$best$AIC$AICc)])
# Adjusted R^2 of the model with the smallest AICc value
RsquareAdj(
  R2.del, 
  n = nrow(mite.h.det), 
  m = which.min(mite.del.res2$best$AIC$AICc)
)
...
# Delaunay triangulation and model selection
(mite.delw <- listw.candidates(mite.xy))
mite.del.res3 <- 
  listw.select(mite.h.det, 
         mite.delw, 
         MEM.autocor = "positive")
# Summary of the results for the best model
summary(mite.del.res3$best)
# Unadjusted R^2 of the model with the smallest AICc value
(R2.del <- 
  mite.del.res3$best$AIC$R2[which.min(mite.del.res2$best$AIC$AICc)])
# Adjusted R^2 of the model with the smallest AICc value
RsquareAdj(
  R2.del, 
  n = nrow(mite.h.det), 
  m = which.min(mite.del.res3$best$AIC$AICc)
)


### END attempts with the new functions.




### From here on no change is necessary ref. test.W ###############################
...
# Space-time interaction analysis in multivariate ANOVA, without replicates =========

# Important additional information

# The stimodels and quicksti functions should only be used (1) when each site has been
# sampled during each survey, with no missing data, and (2) when there are no replicate
# observations of the space-time points. When there is replication, use a regular
# permutational Manova function such as adonis2.
# When the sites form a one-dimensional spatial transect, or a meandering line such as the
# course of a river, with regular sampling intervals, and the time series has fairly equal
# time intervals, one can use the S and Ti arguments to indicate the number of sites in
# space and the number of surveys along time. The order of the sites in each temporal
# block of the input data file will be taken to correspond to the spatial positions of the
# sites along the transect (from 1 to S), and the order of the time blocks in the data
# file will be taken to indicate the temporal order of the surveys (from 1 to Ti). The
# function will then compute dbMEM eigenfunctions corresponding to the spatial and
# temporal positions of the data rows in the input data file as if they were on straight
# lines.
# When the sites do not form a regular, one-dimensional spatial transect, one must provide
# a file of spatial coordinates of the sites to argument S. Similarly, when the time
# series has unequal time intervals, one must provide a file of temporal coordinates of
# the surveys to argument Ti.
# Alternatively, one can use arguments COD.S or COD.T to provide a matrix of Helmert
# contrasts to be used in the analysis in place of dbMEM eigenfunctions. One can do that,
# for example, when there are only a few surveys along time and it would not be useful to
# represent these few surveys by dbMEM eigenfunctions. That matrix can have the class
# "matrix" or "numeric"; they both work in functions stimodels and quicksti. Arguments
# COD.S and COD.T can also be used to provide matrices containing other types of
# eigenfunctions, for example AEM eigenfunctions, to be used instead of dbMEM matrices
# computed by stimodels or quicksti. However, do not code both the space and time factors
# as Helmert contrasts; that would not leave degrees of freedom for the test of the
# interaction.

# sti analysis of the Trichoptera data
