...
'triplot.rda' <-
  function(
    ...
    #
    'larger.plot' <-
      function(sit.sc, spe.sc, BP.sc, percent, move.origin, ax1, ax2, k) {
    ...
    if (length(res.rda$colsum) == 1) {
      stop(
        "Function triplot.rda is not compatible with results that contain no species scores"
      )
    }
    ...
    BP.sc2 <- cbind(
      BP.sc2,
      matrix(0, nrow = nrow(BP.sc2), ncol = length(eig.val) - k)
    )
    ...
    if (!is.null(res.rda$CCA$centroids)) {
      ...
      centroids.sc2 <- cbind(
        centroids.sc2,
        matrix(0, nrow = nrow(centroids.sc2), ncol = length(eig.val) - k)
      )
    ...
    if (ax1 <= k) {
      if (!arrows.only) {
        ...
        if (quant.env.present & plot.env) {
          # Print arrows and labels for quantitative var.
          arrows(
            0,
            0,
            BP.sc[to.plot, ax1] * mult.arrow,
            BP.sc[to.plot, ax2] * mult.arrow,
            length = ahead,
            angle = aangle,
            col = "blue"
          )
          ...
          if (quant.env.present & !plot.env & label.env) {
            # Only print labels for quant. var.
            text(
              BP.sc[to.plot, ax1] * mult.arrow,
              BP.sc[to.plot, ax2] * mult.arrow,
              labels = rownames(BP.sc)[to.plot],
      ...
      if (arrows.only) {
        arrows(
          0,
          0,
          BP.sc[, ax1] * mult.arrow,
          BP.sc[, ax2] * mult.arrow,
          length = ahead,
          angle = aangle,
          col = "blue"
        )
