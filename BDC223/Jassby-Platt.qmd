---
title: "PI Curves: Jassby and Platt"
---

# The 'vanilla'  hyperbolic tangent model
...
kable(random_rows, format = "html", col.names = c("Replicate", "Light (μmol photons m^-2 s^-1)", "Photosynthesis (mg C m^-2 h^-1)"))
```
...
# Considering the effect of the light compenation point

The **light compensation point** is the irradiance level at which the rate of photosynthesis equals the rate of respiration, resulting in a net photosynthetic rate of zero. Below this point, the organism consumes more energy (via respiration) than it produces through photosynthesis, leading to a net loss of energy. Understanding the LCP is crucial for determining the minimum light conditions necessary for the survival of photosynthetic organisms like kelp.

In the context of the **Jassby and Platt hyperbolic tangent model**, the LCP can be estimated by solving for the irradiance $I$ when the net photosynthetic rate $P(I)$ equals zero:
...
Now, the LCP is the irradiance at which $P(I) = 0$:
...
ggplot(data_all, aes(x = Light, y = Photosynthesis, color = factor(Replicate))) +
  ...
  annotate("text", x = fit_results2[[1]]$LCP, y = min(data_all$Photosynthesis) + 5,
           label = paste0("LCP = ", round(fit_results2[[1]]$LCP, 2)),
...
- **$LCP$:** `r round(mean(sapply(fit_results2, function(fit) fit$LCP)), 2)` μmol photons m⁻² s⁻¹
